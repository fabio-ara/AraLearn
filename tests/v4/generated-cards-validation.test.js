...
test("choice com answerIds inválido falha", () => {
  const result = structural({
    position: 1,
    resource: "choice",
    kind: "exercise",
  ...
  assert.equal(result.ok, false);
  assert.match(result.structuralErrors.join("\n"), /answerIds.*answerId inexistente/);
});
...
test("graph rejeita qualquer coordenada autoral", () => {
  const result = structural({
    layout: "auto",
    position: 1,
    resource: "graph",
    ...
    title: "Grafo",
    prompt: "Observe.",
    vertices: [{ id: "a", label: "A", x: 10 }],
    edges: [],
    after: ""
  });
  assert.equal(result.ok, false);
  assert.match(result.structuralErrors.join("\n"), /campo fora do schema: x|Campo fora do schema: "x"/);
});
