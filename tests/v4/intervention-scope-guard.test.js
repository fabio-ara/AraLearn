  ...
  project.courses[0].modules[0].lessons[0].microsequences[0].cards = [
    {
      ...
      title: "Card composto",
      blocks: [
        { id: "paragraph-1", kind: "paragraph", value: "Primeiro bloco." },
        { id: "code-1", kind: "code", prompt: "Observe.", language: "text", code: "segundo" },
        { id: "paragraph-2", kind: "paragraph", value: "Terceiro bloco." }
      ],
      after: "Fechamento."
    },
    {
...
test("snapshot granular registra card e IDs estáveis dos blocos em ordem canônica", () => {
  const project = granularProjectFixture();
  const snapshot = granularSnapshot(project, {
    level: "blocks",
    cardKey: "card-composite",
    blockIds: ["paragraph-2", "paragraph-1"]
  });
  ...
  assert.deepEqual(snapshot.target, {
    ...
    blocks: [
      ...
      { targetId: "paragraph-2", blockKind: "paragraph" }
    ]
  });
...
test("seleção granular rejeita lista vazia, repetição, card ausente e bloco inexistente", async (context) => {
  ...
  const cases = [
    {
      name: "lista vazia",
      target: { level: "blocks", cardKey: "card-composite", blockIds: [] }
    },
    {
      name: "ID repetido",
      target: { level: "blocks", cardKey: "card-composite", blockIds: ["code-1", "code-1"] }
    },
    {
      name: "card ausente",
      target: { level: "blocks", cardKey: "card-absent", blockIds: ["paragraph-1"] }
    },
    {
      name: "bloco ausente",
      target: { level: "blocks", cardKey: "card-composite", blockIds: ["block-absent"] }
    }
  ];
...
test("intervenção em múltiplos blocos aceita os selecionados e preserva os demais", () => {
  ...
  assert.deepEqual(result.blockIds, ["paragraph-1", "paragraph-2"]);
...
test("intervenção no card inteiro permite conteúdo e metadados, mas preserva identidade e ordem", () => {
  ...
  card.blocks = [{ id: "paragraph-new", kind: "paragraph", value: "Nova composição." }];
