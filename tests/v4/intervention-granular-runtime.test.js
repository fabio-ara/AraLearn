...
function compositeCard() {
  return {
    ...
    title: "Conjunção",
    blocks: [
      { id: "paragraph-1", kind: "paragraph", value: "A conjunção exige duas proposições verdadeiras." },
      { id: "code-1", kind: "code", prompt: "Observe a notação.", language: "text", code: "P ∧ Q" },
      { id: "paragraph-2", kind: "paragraph", value: "Apenas V e V produz V." }
    ],
...
test("runtime granular atualiza um card e envia somente o recorte contextual de leitura", async () => {
  let providerRequest;
  ...
  const provider = createFakeProvider({
    script: {
      bottom_up_atomic_resource_patch: (request) => {
        providerRequest = request;
        return {
          text: JSON.stringify({
            replacements: [{ targetId: "card-composite", value: replacement }]
          }),
          usage: {}
        };
      }
  ...
  assert.equal(result.generationResult.patch.kind, "replace-card");
  ...
  assert.equal(providerRequest.engineContext.writableTarget.targetId, "card-composite");
  assert.equal(providerRequest.engineContext.writableTarget.value.resource, "composite");
  assert.equal(providerRequest.prompt.includes("Conteúdo que não pertence ao pedido granular"), true);
...
test("runtime granular atualiza um ou vários blocos e devolve patch restrito", async () => {
  ...
  assert.equal(result.status, "success");
  assert.equal(result.generationResult.patch.kind, "replace-card-blocks");
  assert.deepEqual(result.generationResult.patch.target.blockIds, ["paragraph-1", "paragraph-2"]);
  ...
  assert.deepEqual(
    requestContext.writableTarget.map(({ targetId }) => targetId),
    ["paragraph-1", "paragraph-2"]
  );
  assert.deepEqual(
    requestContext.writableTarget.map(({ value }) => value.kind),
    ["paragraph", "paragraph"]
  );
  assert.deepEqual(
    requestContext.readOnlyContext.unselectedBlocks.map(({ id, kind }) => ({ id, kind })),
    [{ id: "code-1", kind: "code" }]
  );
  assert.equal(Object.hasOwn(requestContext, "projectDocument"), false);
});

test("runtime bloqueia retorno que tenta alcançar bloco não selecionado", async () => {
  const provider = createFakeProvider({
    script: {
      bottom_up_atomic_resource_patch: {
        text: JSON.stringify({
          replacements: [{
            targetId: "code-1",
            value: { id: "code-1", kind: "code", prompt: "Alterado.", language: "text", code: "P ∨ Q" }
          }]
        })
      }
    }
...
test("falha transitória preserva o escopo e a retomada aplica a mesma seleção", async () => {
            ...
            text: JSON.stringify({
              replacements: [{
                ...
                value: {
                  id: "paragraph-1",
                  kind: "paragraph",
                  value: "Explicação recuperada após a retomada."
                }
              }]
            })
          };
  ...
  assert.deepEqual(resumed.generationResult.patch.target.blockIds, ["paragraph-1"]);
});
