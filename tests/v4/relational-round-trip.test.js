...
test("fixture visual faz round-trip exato sem persistir geometria de grafo", async () => {
  const source = await readJson(fixture("v4/project-visual.json"));
  ...
  assert.ok(rows.nodes.some((row) => row.nodeScope === "graph"));
  assert.equal(rows.nodes.some((row) => row.nodeScope === "graph" && (row.hasX || row.hasY)), false);
...
test("todos os recursos, blocos e filhos reais preservam igualdade semântica", async () => {
  ...
  courses.push(buildDisciplinaryScenarioProject().courses[0]);
...
test("fragmento de microssequência faz round-trip sem scaffolding documental", async () => {
  const project = await readJson(fixture("v4/project-minimal.json"));
...
test("campos aninhados sem mapeamento são rejeitados em vez de desaparecer", async () => {
  const project = await readJson(fixture("v4/project-visual.json"));
