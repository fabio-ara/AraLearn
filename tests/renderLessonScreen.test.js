...
test("renderiza prévia pendente da assistência sem aplicar os cards automaticamente", () => {
  ...
  assert.match(html, /Rascunho privado pendente/);
...
test("renderiza alternância para conteúdo oficial quando existe rascunho privado pendente", () => {
  const project = readProject();
  const course = project.courses[0];
  const moduleValue = course.modules[0];
  const lesson = moduleValue.lessons[0];
  ...
  assert.match(html, /Conteúdo oficial em uso/);
  assert.match(html, /data-action="show-assist-preview"/);
  assert.match(html, /Gemini 2\.5 Flash/);
  assert.match(html, /12\/05 12:00/);
  assert.match(html, /editor-card-count-value">1\/7<\/span>/);
});
