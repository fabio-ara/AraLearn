...
test("curso oficial permanece compartilhado até existir uma alteração autoral", async (context) => {
  let forkCalls = 0;
  const indexedDb = new IDBFactory();
  const store = await IndexedDbRelationalStore.open(indexedDb, { userId: TEST_USER_ID });
  const { course, selection } = await seedSelectedOfficialCourse(store);
  const originalOfficialBlocks = await store.getAll("blocks");
  const forkCourseForEditing = async (sourceCourseId) => {
    forkCalls += 1;
    ...
    personalCourse.sourceCourseId = course.id;
  ...
  assert.deepEqual(repository.coursePermissions(course.contractKey), {
    role: "learner",
    canEdit: true,
    canDelete: false,
    requiresFork: true
  });
  assert.equal(repository.canEditCourse(course.contractKey), true);
  assert.equal(repository.canDeleteCourse(), false);

  // Abrir, estudar ou salvar um documento semanticamente idêntico não cria
  // árvore pessoal nem coloca o catálogo na outbox.
  await repository.saveProject(repository.loadProject());
  assert.equal(forkCalls, 0);
  assert.equal((await store.get("courses", course.id)).kind, undefined);
  assert.deepEqual(await store.getAll("outbox"), []);

  const edited = repository.loadProject();
  edited.courses[0].modules[0].lessons[0].microsequences[0].cards[0].text =
    "Texto personalizado apenas neste bloco.";
  ...
  assert.equal(personalCourse.ownerId, TEST_USER_ID);
  assert.equal(personalCourse.sourceCourseId, course.id);

  const firstOutbox = await store.getAll("outbox");
  assert.equal(firstOutbox.length, 1);
  assert.equal(firstOutbox[0].entityType, "blocks");
  assert.equal(firstOutbox[0].courseId, personalCourse.id);
  assert.deepEqual(firstOutbox[0].changedFields, ["value"]);
  assert.deepEqual(firstOutbox[0].payload, {
    value: "Texto personalizado apenas neste bloco."
  });
  assert.deepEqual(
    (await store.getAll("blocks")).filter((row) => row.courseId === course.id),
    originalOfficialBlocks
  );

  // Edições seguintes reutilizam a mesma árvore pessoal; copy-on-write ocorre
  // uma única vez e não duplica o curso a cada gravação.
  const editedAgain = repository.loadProject();
  editedAgain.courses[0].modules[0].lessons[0].microsequences[0].cards[0].text =
    "Segunda edição no mesmo bloco.";
  await repository.saveProject(editedAgain);
  assert.equal(forkCalls, 1);
  assert.equal((await store.getAll("courses")).filter((row) => row.kind === "personal").length, 1);
  assert.deepEqual(
    (await store.getAll("outbox")).map((entry) => entry.entityType),
    ["blocks", "blocks"]
  );
});
