...
test("curso oficial é somente leitura no aplicativo estudantil", async (context) => {
  const { repository, store } = await openSelectedCourseRepository(new IDBFactory());
  context.after(() => store.close());

  assert.deepEqual(repository.coursePermissions(), {
    role: "learner",
    canEdit: false,
    canDelete: false
  });
  assert.equal(repository.canEditCourse(), false);
  ...
  edited.courses[0].title = "Edição local indevida";
  assert.throws(
    () => repository.saveProject(edited),
    /somente para estudo/u
  );
  assert.throws(
    () => repository.replaceMicrosequenceCards(edited, "micro-fixture-minimal"),
    /somente para estudo/u
  );
  assert.deepEqual(repository.loadProject(), minimalProjectFixture);
});
