...
test("choice, graph, relation_map, tree, matrix e plane compilam estruturas válidas", () => {
  ...
  assert.equal(matrixChoice.answer, "b");
...
test("matrix_locate_cell_choice deriva answer e rejeita inconsistências semânticas", () => {
  assert.equal(compileCardFromTemplate({
    templateId: "matrix_locate_cell_choice",
    position: 1,
    slots: {
      1: "Posição",
...
test("graph_simple calcula caminho único válido e corrige answer inconsistente", () => {
  const card = compileCardFromTemplate({
    templateId: "graph_simple",
    position: 1,
    slots: {
