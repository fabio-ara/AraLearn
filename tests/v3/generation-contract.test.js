...
test("o reparo mecânico normaliza exercício contextual choice e graph com nodes/links", () => {
  const planningContract = buildPlanningContract();
  const { validatedPlan } = buildValidatedDraftPlan({
    planningContract,
    type: "guided_practice",
    size: "short",
    draft: [
      { position: 1, resource: "paragraph", kind: "theory", exercise: "none", goal: "Mostrar a relação principal." },
      { position: 2, resource: "graph", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura correta do caso." },
      { position: 3, resource: "paragraph", kind: "exercise", exercise: "gap", goal: "Fechar com lacuna curta." }
    ]
  });
  const generationContract = buildMicrosequenceGenerationContract({
    planningContract,
    ...
    cards: [
      {
        ...
        exercise: "none",
        title: "Regra",
        text: "A relação principal aparece no caso mostrado.",
        after: ""
      },
      {
        position: 2,
        ...
        prompt: "Qual leitura está correta?",
        nodes: [
          { id: "A", name: "A" },
          { id: "B", label: "B" }
        ],
        ...
        options: ["A está ligado a B", { value: "b", label: "B está ligado a A" }, { id: "c", content: "Não há ligação" }],
        answer: "A está ligado a B",
        after: ""
      },
      {
        position: 3,
        ...
        exercise: "gap",
        title: "Complete",
        text: "Complete [[A liga B::A liga B|B liga A|não há ligação]].",
        after: ""
      }
    ]
  }, generationContract);
  ...
  assert.deepEqual(repaired.cards[1].vertices.map((item) => item.id), ["A", "B"]);
  assert.deepEqual(repaired.cards[1].options.map((item) => item.id), ["a", "b", "c"]);
  assert.equal(repaired.cards[1].answer, "a");
});

test("o reparo mecânico completa lacuna curta sem :: e neutraliza pergunta que revela a resposta", () => {
  const planningContract = buildPlanningContract();
  const { validatedPlan } = buildValidatedDraftPlan({
    planningContract,
    type: "concept",
    ...
    draft: [
      { position: 1, resource: "paragraph", kind: "theory", exercise: "none", goal: "Explicar a regra." },
      { position: 2, resource: "table", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura de uma linha da tabela." },
      { position: 3, resource: "table", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura da tabela." }
    ]
  });
  const generationContract = buildMicrosequenceGenerationContract({
    planningContract,
  ...
  const repaired = repairGeneratedCardsDeterministic({
    cards: [
      ...
      },
      {
        ...
        columns: ["P", "Q", "P ∧ Q"],
        rows: [["V", "V", "V"], ["V", "F", "F"]],
        question: "Qual linha em que P e Q são verdadeiras confirma a regra principal?",
        options: [
          { id: "a", text: "A linha em que P e Q são verdadeiras" },
          { id: "b", text: "A linha em que só P é verdadeira" },
          { id: "c", text: "A linha em que só Q é verdadeira" }
        ],
        answer: "a",
        after: ""
      },
      {
        ...
        columns: ["P", "Q", "P ∧ Q"],
        rows: [["V", "V", "V"], ["V", "F", "F"]],
        question: "Se P é verdadeiro e Q é falso, qual é o valor lógico de P ∧ Q?",
        options: [
          { id: "a", text: "Verdadeiro" },
          { id: "b", text: "Falso" },
          { id: "c", text: "Não é possível determinar" }
        ],
        answer: "b",
        after: ""
      }
  ...
  const normalizedGap = repairGeneratedCardsDeterministic({
    cards: [
      {
        position: 1,
        resource: "paragraph",
        ...
        exercise: "gap",
        title: "Complete",
        text: "Complete [[ambas são verdadeiras|pelo menos uma é verdadeira|nenhuma é verdadeira]].",
        after: ""
      }
    ]
  }, {
    ...generationContract,
    plan: [
      { position: 1, role: "practice", resource: "paragraph", kind: "exercise", exercise: "gap", goal: "Completar a regra.", checks: [] }
    ],
    output: { format: "json", cardCount: 1 }
  });
  ...
  assert.equal(repaired.cards[1].question, "Qual opção corresponde corretamente ao caso mostrado?");
  assert.equal(repaired.cards[2].question, "Qual opção corresponde corretamente ao caso mostrado?");
  assert.equal(normalizedGap.cards[0].text.includes("[[ambas são verdadeiras::ambas são verdadeiras|pelo menos uma|nenhuma é verdadeira]]"), true);
});

test("o reparo mecânico converte paragraph gap malformado com campos de quiz em lacuna válida", () => {
  const planningContract = buildPlanningContract();
  const { validatedPlan } = buildValidatedDraftPlan({
    planningContract,
    type: "concept",
    size: "short",
    draft: [
      { position: 1, resource: "paragraph", kind: "theory", exercise: "none", goal: "Explicar a ideia." },
      { position: 2, resource: "paragraph", kind: "exercise", exercise: "gap", goal: "Cobrar a ideia principal." },
      { position: 3, resource: "choice", kind: "exercise", exercise: "choice", goal: "Consolidar a leitura." }
    ]
  });
  const generationContract = buildMicrosequenceGenerationContract({
    planningContract,
    ...
    cards: [
      {
        ...
        exercise: "none",
        title: "Base",
        text: "O PC guarda o endereço da próxima instrução.",
        after: ""
      },
      {
        position: 2,
        ...
        exercise: "gap",
        title: "Lacuna",
        question: "O que o PC guarda?",
        options: [
          { id: "a", text: "o endereço de memória da próxima instrução" },
          { id: "b", text: "o resultado final" },
          { id: "c", text: "a instrução completa" }
        ],
        answer: "a",
        after: ""
      },
      {
        ...
        options: [
          { id: "a", text: "O PC guarda endereço." },
          { id: "b", text: "O PC guarda a instrução." },
          { id: "c", text: "O PC guarda o resultado." }
        ],
        answer: "a",
        after: ""
      }
  ...
  assert.match(repaired.cards[1].text, /\[\[[^\]]+::[^\]]+\]\]/);
  assert.equal("question" in repaired.cards[1], false);
  assert.equal("options" in repaired.cards[1], false);
  assert.equal("answer" in repaired.cards[1], false);
...
test("o reparo mecânico neutraliza pergunta quando a resposta correta curta reaparece inteira", () => {
  const planningContract = buildPlanningContract();
  const { validatedPlan } = buildValidatedDraftPlan({
    planningContract,
    type: "guided_practice",
    ...
    draft: [
      { position: 1, resource: "paragraph", kind: "theory", exercise: "none", goal: "Explicar a ideia." },
      { position: 2, resource: "choice", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura do caso." },
      { position: 3, resource: "choice", kind: "exercise", exercise: "choice", goal: "Consolidar a leitura." }
    ]
  });
  const generationContract = buildMicrosequenceGenerationContract({
    ...
    cards: [
      {
        ...
        exercise: "none",
        title: "Base",
        text: "A memória recebe o endereço do PC.",
        after: ""
      },
      {
        position: 2,
        ...
        exercise: "choice",
        title: "Prática",
        question: "A memória principal recebe diretamente o endereço enviado pelo PC?",
        options: [
          { id: "a", text: "A memória principal" },
          { id: "b", text: "O registrador de instruções" },
          { id: "c", text: "A unidade lógica e aritmética" }
        ],
        answer: "a",
        after: ""
      },
  ...
  assert.equal(repaired.cards[1].question, "Qual opção está correta?");
  const validation = validateGeneratedCards(repaired, generationContract);
  assert.equal(
    validation.didacticErrors.some((error) => String(error).includes("repete o mesmo caso") || String(error).includes("reaproveita o mesmo caso concreto")),
    false
...
test("o reparo mecânico normaliza plane com pares numéricos em string", () => {
  const planningContract = buildPlanningContract();
  const validatedPlan = {
    plan: {
      type: "concept",
      size: "short",
      goal: "Representar um vetor como seta no plano.",
      extraResources: ["plane"],
      sources: [],
      reason: "Plano local validado.",
      cardPlan: [
        { position: 1, role: "explain", resource: "plane", kind: "theory", exercise: "none", goal: "Apresentar o vetor no plano.", checks: [] },
        { position: 2, role: "practice", resource: "plane", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura do caso.", checks: [] },
        { position: 3, role: "next", resource: "paragraph", kind: "theory", exercise: "none", goal: "Consolidar a ideia.", checks: [] }
      ]
    }
  };
  const generationContract = buildMicrosequenceGenerationContract({
    ...
    cards: [
      {
        ...
        exercise: "none",
        title: "Plano",
        prompt: "Observe o vetor no plano.",
        x: ["0", "0"],
        y: ["0", "0"],
        vector: ["2", "5"],
        after: ""
      },
      {
        ...
        prompt: "Observe o vetor no plano.",
        vector: "(3, 4)",
        question: "Qual opção corresponde ao caso mostrado?",
        options: [
          { id: "a", text: "(3, 4)" },
          { id: "b", text: "(4, 3)" },
          { id: "c", text: "(0, 7)" }
      ...
      },
      {
        ...
        exercise: "none",
        title: "Fechamento",
        text: "A seta vai da origem ao ponto indicado pelo vetor.",
        after: ""
      }
    ]
  }, generationContract);

  assert.deepEqual(repaired.cards[0].vector, [2, 5]);
  assert.deepEqual(repaired.cards[1].vector, [3, 4]);
...
test("o reparo mecânico remove x e y inválidos de plane quando já há vector válido", () => {
  const planningContract = buildPlanningContract();
  const validatedPlan = {
    plan: {
      type: "concept",
      ...
      reason: "Plano local validado.",
      cardPlan: [
        { position: 1, role: "practice", resource: "plane", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura do caso.", checks: [] },
        { position: 2, role: "practice_more", resource: "choice", kind: "exercise", exercise: "choice", goal: "Variar a prática.", checks: [] },
        { position: 3, role: "next", resource: "paragraph", kind: "theory", exercise: "none", goal: "Consolidar a ideia.", checks: [] }
      ]
    }
  };
  ...
  const repaired = repairGeneratedCardsDeterministic({
    cards: [
      ...
      },
      {
        ...
        exercise: "none",
        title: "Fechamento",
        text: "A leitura correta preserva as componentes do vetor.",
        after: ""
      }
    ]
  }, generationContract);

  assert.equal("x" in repaired.cards[0], false);
  assert.equal("y" in repaired.cards[0], false);
  assert.deepEqual(repaired.cards[0].vector, [4, -1]);
...
test("o reparo mecânico indenta code multilinha sem indentação mínima", () => {
  const planningContract = buildPlanningContract();
  const { validatedPlan } = buildValidatedDraftPlan({
    planningContract,
    ...
    size: "short",
    draft: [
      { position: 1, resource: "paragraph", kind: "theory", exercise: "none", goal: "Explicar a estrutura." },
      { position: 2, resource: "code", kind: "exercise", exercise: "choice", goal: "Cobrar a leitura do bloco." },
      { position: 3, resource: "choice", kind: "exercise", exercise: "choice", goal: "Consolidar a leitura." }
    ]
  });
  const generationContract = buildMicrosequenceGenerationContract({
    ...
    cards: [
      {
        ...
        exercise: "none",
        title: "Regra",
        text: "A linha indentada pertence ao bloco aberto.",
        after: ""
      },
      {
        position: 2,
        ...
        exercise: "choice",
        title: "Prática",
        prompt: "Observe o bloco.",
        language: "java",
        code: "if (x > 0) {\nSystem.out.println(x);\n}",
        question: "Qual linha fica dentro do bloco?",
        options: [
          { id: "a", text: "System.out.println(x);" },
          { id: "b", text: "if (x > 0)" },
          { id: "c", text: "}" }
        ],
        answer: "a",
      ...
      },
      {
        ...
        question: "Qual opção está correta?",
        options: [
          { id: "a", text: "A linha indentada fica dentro do bloco." },
          { id: "b", text: "Não há bloco no exemplo." },
          { id: "c", text: "A chave fecha antes da impressão." }
        ],
        answer: "a",
        after: ""
      }
