...
import { TEMPLATE_CATALOG } from "../../src/generation/engine/templateCatalog.js";
...
test("catálogos de recursos permanecem idênticos no domínio, no relacional, na geração e na autoria", async () => {
  ...
  assert.deepEqual(sorted(generationTemplates), canonical);
  assert.deepEqual(
  ...
  for (const resource of RESOURCE_CATALOG) {
    assert.ok(resource.templates.length > 0, `${resource.id} precisa declarar ao menos um template.`);
    resource.templates.forEach((templateId) => {
      assert.equal(TEMPLATE_CATALOG[templateId]?.resource, resource.id, `${templateId} divergiu de ${resource.id}.`);
    });
  }
});
