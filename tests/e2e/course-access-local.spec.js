...
test.describe("acesso direto de Curso no Supabase local", () => {
  ...
  test("visitante abre endereço público, pratica localmente e baixa somente o PDF autorizado", async ({ browser }, testInfo) => {
    ...
    try {
      ...
      if (!publicStudyDiagnostic.body.includes("Conteúdo privado liberado somente para a pessoa escolhida.")) {
        throw new Error(`PUBLIC_STUDY_DIAGNOSTIC ${JSON.stringify(publicStudyDiagnostic)}`);
      }
