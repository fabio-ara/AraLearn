...
test("Análise: distribuição, comparação, teclado e exportação nas oito geometrias", async ({ page }, info) => {
  ...
  for (const width of [360, 390, 430, 1280]) for (const theme of ["light", "dark"]) {
    ...
    const headingBox = await panel.getByRole("heading", { name: "Novidade declarada" }).boundingBox();
    expect(headingBox.x).toBeGreaterThanOrEqual(8);
    expect(headingBox.y).toBeGreaterThanOrEqual(8);
    expect(headingBox.x + headingBox.width).toBeLessThanOrEqual(width - 8);
    await expect(panel).not.toContainText("edição 7");
    ...
    await expect(dialog.getByRole("combobox", { name: "Parte do curso", exact: true })).toBeVisible();
    expect(await dialog.getByRole("combobox", { name: "Parte do curso", exact: true }).evaluate(node => getComputedStyle(node).fontWeight)).toBe("400");
    await dialog.getByRole("button", { name: "Comparar cursos", exact: true }).click();
    await expect(dialog.getByText("Inventário planejado", { exact: true })).toBeVisible();
    const summaries = dialog.locator("fieldset > details > summary");
    const rows = await summaries.evaluateAll(nodes => nodes.map(node => ({ height: node.getBoundingClientRect().height, display: getComputedStyle(node).display })));
    expect(new Set(rows.map(row => row.height)).size).toBe(1);
    expect(rows.every(row => row.display === "list-item")).toBe(true);
    await expect(dialog.getByRole("button", { name: "Comparar cursos", exact: true })).toHaveText("");
