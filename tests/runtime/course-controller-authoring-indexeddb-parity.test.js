...
test("entrega por clipboard não altera o IndexedDB nem o snapshot remoto de Autoria", async (context) => {
  const indexedDb = new IDBFactory();
  ...
  const api = {
    ...
    async loadAuthoringPlan() {
      ...
      return {
        ...
        courseId: COURSE_ID,
        courseRevision: 4,
        plan: { id: COURSE_ID, version: 4, parts: [] },
        recentActivity: []
      };
    },
    async loadAuthoringOutline() {
      remoteReads += 1;
      return { courseId: COURSE_ID, revision: 4, outline: { modules: [] } };
    },
    async loadAuthoringStudyUnits() {
      remoteReads += 1;
      return inspectionPage(4);
    }
  };
  const delivered = [];
  ...
  await controller.loadAuthoringPlan(COURSE_ID);
  await controller.loadAuthoringOutline(COURSE_ID);
  ...
  const before = await Promise.all(cacheKeys.map((key) => store.getCache(key)));
  const readsBeforeDelivery = remoteReads;
  const request = {
    requestId: "request-chat-parity-0001",
    requestText: "Revise esta Lição e devolva ao mesmo Curso.",
    context: {
      course: { id: COURSE_ID, title: "Curso", revision: 4 },
      target: { type: "lesson", id: "lesson-a", title: "Lição A" }
    }
  };

  assert.deepEqual(await controller.requestAuthoringRequest(request), {
    delivery: "clipboard",
    message: "Pedido copiado."
  });
  assert.equal(remoteReads, readsBeforeDelivery);
  assert.deepEqual(
    await Promise.all(cacheKeys.map((key) => store.getCache(key))),
    before
  );
  assert.deepEqual(delivered, [request]);
  assert.notEqual(delivered[0], request);
});
