...
test("prova Android distingue Configurações da candidata e da base e recusa perda de identidade, bytes, licença ou tema (sintético)", () => {
...
test("Pages e Release exigem prova do APK exato, sem reconstrução ou permissões de assinatura no emulador", () => {
  ...
  assert.match(gate, /device\.call\("install", str\(candidate\)[\s\S]+device\.launch\(\)\s+device\.wait_label\("Configurações"\)\s+device\.capture\("clean-initial"\)\s+device\.isolate_network\(\)\s+device\.dark\(\)/u);
  ...
  assert.match(gate, /device\.call\("install", "-r", str\(candidate\)[\s\S]+device\.launch\(\)\s+device\.wait_label\("Configurações"\)\s+device\.isolate_network\(\)/u);
