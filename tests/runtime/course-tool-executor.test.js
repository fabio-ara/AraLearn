...
test("conhecimento mantém núcleo curto e carrega orientação somente por fase", () => {
  ...
  assert.match(materialization.text, /Antes da primeira StudyUnit.*inventário necessário.*completo/iu);
  assert.match(materialization.text, /recorte focal.*novidades que ela pode introduzir/iu);
  assert.match(materialization.text, /conhecimentos explicitamente estabelecidos.*função didática/iu);
  assert.match(materialization.text, /Antes de gravar.*novidade material não inventariada/iu);
  assert.match(materialization.text, /termo geral incidental não vira unidade de análise/iu);
  assert.match(materialization.text, /Distribua profundidade/iu);
  assert.match(materialization.text, /Não há meta de quantidade total de StudyUnits/iu);
  assert.match(materialization.text, /limite editorial produz mais Unidades/iu);
  ...
  assert.match(planning.text, /Antes de escrever qualquer StudyUnit.*inventarie toda novidade/iu);
  assert.match(planning.text, /Conceitos auxiliares.*operações intelectuais/iu);
  assert.match(planning.text, /Curso assume explicitamente.*já introduziu/iu);
  assert.match(planning.text, /termo especializado novo.*não o transforma em pré-requisito/iu);
  ...
  assert.match(planning.text, /comparar tetos.*preserve o mesmo inventário/iu);
  assert.match(planning.text, /nunca agregue statements para fazê-los caber/iu);
