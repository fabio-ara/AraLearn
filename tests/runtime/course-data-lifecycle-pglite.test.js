...
const SOURCE_COURSE = "20000000-0000-4000-8000-000000000002";
...
test("cota de PDF reserva anexos, objetos e intents vivos por hash único", async () => {
  const database = new PGlite();
  const twentyMiB = 20 * 1024 * 1024;
  const fourMiB = 4 * 1024 * 1024;
  const hashes = Object.fromEntries(
    ["a", "b", "c", "d", "e", "f"].map((digit) => [digit, digit.repeat(64)])
  );
  await database.exec(`
    create schema private;
    ...
    insert into private.course_source_attachments(
      ...
      '${COURSE}','source-quota',1,'${hashes.a}',${twentyMiB},
      'application/pdf','${COURSE}/${hashes.a}.pdf'
    );
    insert into storage.objects(bucket_id,name,metadata) values
      ('course-source-pdfs','${COURSE}/${hashes.a}.pdf',
        '{"size":${twentyMiB},"mimetype":"application/pdf"}'::jsonb),
      ('course-source-pdfs','${COURSE}/${hashes.b}.pdf',
        '{"size":${twentyMiB},"mimetype":"application/pdf"}'::jsonb);
    ...
    (
      '${OWNER}','${COURSE}','${COURSE}/${hashes.c}.pdf','${hashes.c}',
      ${twentyMiB},'application/pdf','source-quota',1,7,now(),now()+interval '5 minutes'
    ),(
      '${OWNER}','${COURSE}','${COURSE}/${hashes.f}.pdf','${hashes.f}',
      ${twentyMiB},'application/pdf','source-quota',1,7,
  ...
  let reserved = await database.query(
    "select private.course_source_pdf_reserved_bytes_v1($1) value",
    [COURSE]
  );
  assert.equal(Number(reserved.rows[0].value), 3 * twentyMiB);

  const prepare = (hash, byteSize) => database.query(`
    select public.get_course_source_attachment_access_for_actor_v1(
      $1,$2,7,'prepare_upload','source-quota',1,$3,$4,'application/pdf'
    ) value
  `, [OWNER, COURSE, hash, byteSize]);
  const first = await prepare(hashes.d, fourMiB);
  assert.equal(first.rows[0].value.uploadRequired, true);
  const duplicate = await prepare(hashes.d, fourMiB);
  assert.equal(duplicate.rows[0].value.uploadRequired, true);
  reserved = await database.query(
    "select private.course_source_pdf_reserved_bytes_v1($1) value",
    [COURSE]
  );
  assert.equal(Number(reserved.rows[0].value), 64 * 1024 * 1024);
  await assert.rejects(
    prepare(hashes.e, 1),
...
test("acesso final ao PDF preserva o objeto herdado por uma Variante", async () => {
  const database = new PGlite();
  const hash = "a".repeat(64);
  const inheritedPath = `${SOURCE_COURSE}/${hash}.pdf`;
  await database.exec(`
    create role anon;
    create role authenticated;
    create role service_role;
    ...
    returns bigint language sql stable as $$select 0::bigint$$;
    create function private.course_source_pdf_reserved_bytes_v1(p_course_id uuid)
    returns bigint language sql stable as $$select 0::bigint$$;
    create function private.valid_course_source_pdf_object_v1(
      p_storage_path text,p_byte_size bigint,p_media_type text
    ...
    values('${COURSE}','${OWNER}',7);
    insert into private.course_source_revisions(course_id,source_id,revision,status)
    values('${COURSE}','source-inherited',3,'active');
    insert into private.course_source_attachments(
      course_id,source_id,source_revision,content_hash,byte_size,media_type,storage_path
    ) values(
      '${COURSE}','source-inherited',3,'${hash}',1024,'application/pdf','${inheritedPath}'
    );
    insert into storage.objects(bucket_id,name)
    values('course-source-pdfs','${inheritedPath}');
  `);
  await database.exec(await migrationSlice(
    "create or replace function public.get_course_source_attachment_access_for_actor_v1(",
    "-- A exclusao invalida"
  ...
  for (const operation of ["prepare_upload", "download"]) {
    const result = await database.query(`
      select public.get_course_source_attachment_access_for_actor_v1(
        $1,$2,7,$3,'source-inherited',3,$4,
        case when $3='prepare_upload' then 1024 else null end,
        case when $3='prepare_upload' then 'application/pdf' else null end
      ) value
    `, [OWNER, COURSE, operation, hash]);
    assert.equal(result.rows[0].value.storageOriginCourseId, SOURCE_COURSE);
    assert.equal(result.rows[0].value.attachment.storagePath, inheritedPath);
    assert.equal(result.rows[0].value.uploadRequired, false);
    assert.equal(result.rows[0].value.alreadyLinked, true);
    assert.equal(
      result.rows[0].value.contract,
      operation === "download"
