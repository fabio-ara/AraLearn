...
export const COURSE_AUTHORING_SERVER_INSTRUCTIONS = [
  ...
  "Retome o contexto. Foco Conteúdo desenvolve explicação e fontes antes das unidades. Ensine dependências antes do uso. Cadência, revisão e diálogo são independentes. Parte é lote operacional, não uma dependência pedagógica por si só. Verifique o mesmo lote candidato antes da escrita; resolva autonomamente condições deriváveis e escolhas delegadas. Na conversa normal, nunca exponha nomes de operações, códigos, estados internos, tokens ou referências opacas. Se restar uma decisão humana real, consolide pendências equivalentes, explique a dependência em linguagem de aprendizagem, faça uma única pergunta e depois retome a intenção original. Prática exige avaliação e feedback offline. Pendências fora do alvo não bloqueiam produção focal. Curso público exige definir_visibilidade e releitura antes do sucesso. Use link exato em Markdown e próxima etapa no mandato."
].join("\n");
