...
function nodeStatement(node) {
  ...
  if (!nodeStereotype(node)) attributes.xlabel = wrapGraphvizLabel(node.label, 24);
...
function labels(data) {
  const boundaries = data.participants.flatMap((participant) => [{
    kind: "boundary",
    id: `participant-${participant.id}`,
    graphvizId: `bpmn-participant-${participant.id}`,
    plain: participant.label,
    html: `<span>${renderPackageInline(participant.label)}</span>`
  }, ...participant.lanes.map((lane) => ({
    kind: "boundary",
    id: `lane-${participant.id}-${lane.id}`,
    graphvizId: `bpmn-lane-${participant.id}-${lane.id}`,
    plain: lane.label,
    html: `<span>${renderPackageInline(lane.label)}</span>`
  }))]);
  return [...boundaries, ...data.nodes.map((node) => {
    const stereotype = nodeStereotype(node);
    if (!stereotype) return { kind: "external", id: node.id, graphvizId: `system-node-${node.id}`,
      textStart: node.kind.endsWith("gateway") ? 1 : 0, plain: node.label,
...
export const bpmnProcessPackage = Object.freeze({
  manifest: Object.freeze({ id: "aralearn.resource.bpmn_process", version: "1.0.0", label: "Processo BPMN", purpose: "Representar participantes, raias, eventos, atividades, gateways e fluxos segundo o subconjunto didático de BPMN 2.0.", slots: Object.freeze(["content", "feedback"]), taskOperations: Object.freeze(["trace-business-process", "distinguish-responsibility", "inspect-gateway", "compare-message-and-sequence-flow"]), academic: academicProfile({ domains: ["gestão de processos", "análise de negócios", "engenharia de software"], knowledgeObjects: ["participante", "raia", "evento", "atividade", "gateway", "fluxo de sequência", "fluxo de mensagem"], conventions: ["eventos circulares", "atividades em retângulos arredondados", "gateways em losangos", "fluxo de sequência contínuo", "fluxo de mensagem tracejado", "responsabilidade delimitada por participante e raia"], appropriateWhen: ["o processo de negócio e a responsabilidade entre participantes são parte da tarefa"], avoidWhen: ["o objeto é um algoritmo de programação", "o percurso depende de estado persistente", "um simples procedimento linear basta"], technologies: ["BPMN 2.0", "Graphviz", "Viz.js WebAssembly", "SVG"], practiceModes: ["exposition", "gap", "typing", "selection"] }), responseCompatibility: Object.freeze(["aralearn.response.gap", "aralearn.response.choice"]), limitations: Object.freeze(["Cobre eventos, tarefas, gateways, raias e fluxos; coreografia e conversação exigem packages próprios.", "Processos extensos devem ser decompostos por subprocesso."]), accessibility: "Participantes, raias, elementos e fluxos possuem descrição textual equivalente." }),
