...
export const relationMapPackage = Object.freeze({
  ...
  render(data) {
    ...
    return `<div class="runtime-block runtime-relation-map-block package-relation-map" data-density="${data.relations.length > 8 ? "high" : "normal"}">${renderPackageProse(data.prompt)}<div class="package-relation-map-headings" aria-hidden="true"><span>${renderPackageInline(data.leftSet.label)}</span><span>${renderPackageInline(data.rightSet.label)}</span></div><ol class="package-relation-map-list" aria-label="Correspondências entre ${escapePackageAttribute(data.leftSet.label)} e ${escapePackageAttribute(data.rightSet.label)}">${data.relations.map((relation) => `<li class="package-relation-row${highlighted.has(relation.id) ? " is-highlighted" : ""}" data-relation-id="${escapePackageAttribute(relation.id)}"><span class="package-relation-item package-relation-left">${renderPackageInline(left.get(relation.from))}</span><span class="package-relation-verb"><span aria-hidden="true">→</span>${relation.label ? `<small>${renderPackageInline(relation.label)}</small>` : ""}</span><span class="package-relation-item package-relation-right">${renderPackageInline(right.get(relation.to))}</span></li>`).join("")}</ol></div>`;
  },
