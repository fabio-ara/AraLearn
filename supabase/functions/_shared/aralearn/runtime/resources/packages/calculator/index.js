...
export const calculatorPackage = Object.freeze({
  ...
  render(data, options = {}) {
    ...
    return `<section class="runtime-block package-calculator"><h3>${escapePackageHtml(data.title)}</h3>${data.prompt ? renderPackageProse(data.prompt) : ""}<form data-calculator-form><label for="${key}">Expressão</label><input id="${key}" data-calculator-input type="text" dir="ltr" maxlength="256" autocomplete="off" autocapitalize="off" spellcheck="false" value="${escapePackageAttribute(data.initialExpression || "")}"><label>Unidade dos ângulos<select data-calculator-angle><option value="radians"${data.angleUnit === "radians" ? " selected" : ""}>Radianos</option><option value="degrees"${data.angleUnit === "degrees" ? " selected" : ""}>Graus</option></select></label><div class="package-calculator-actions"><button type="submit">Calcular</button><button type="button" data-calculator-clear>Limpar</button></div><output data-calculator-output role="status" aria-live="polite" aria-atomic="true"></output></form><details class="package-calculator-limits"><summary>Operações e precisão</summary><p>${limitations}</p><p>Use + − × ÷ ^, parênteses, pi e e. Funções: abs, sqrt, ln, log (base 10), exp, sin, cos e tan. Escreva o argumento entre parênteses e toda multiplicação com * ou ×.</p><p>Ponto ou vírgula decimal; sem separador de milhar. Potências associam à direita: 2^3^2 = 512; −2^2 = −4. Até 256 caracteres, 128 elementos e 32 níveis. Ângulos limitados a ±10¹²; tangente próxima de um polo é recusada.</p></details></section>`;
  },
