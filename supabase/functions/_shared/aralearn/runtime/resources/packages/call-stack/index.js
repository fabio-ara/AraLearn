...
export const callStackPackage = Object.freeze({
  ...
  render(data) {
    ...
    return `<div class="runtime-block package-call-stack">${data.prompt ? renderPackageProse(data.prompt) : ""}<figure><figcaption>A chamada ativa está no topo. Quando termina, o resultado volta ao quadro logo abaixo.</figcaption><ol>${frames.map((frame, index) => `<li data-frame-id="${escapePackageAttribute(frame.id)}" data-frame-state="${index === 0 ? "active" : "suspended"}"${index === 0 ? ' class="is-active" aria-current="step"' : ""}><header><span class="package-call-stack-state">${index === 0 ? "Em execução" : "Suspensa"}</span><strong>${renderPackageInline(frame.functionName)}</strong><small>Chamada ${frames.length - index}</small></header><dl>${frame.fields.map((field) => `<div><dt><span>${renderPackageInline(field.name)}</span><small>${renderPackageInline(FIELD_ROLE_LABELS[field.role] || field.role)}</small></dt><dd><code>${renderPackageInline(field.value || "—")}</code></dd></div>`).join("")}</dl><p class="package-call-stack-return"><span aria-hidden="true">${index < frames.length - 1 ? "↓" : "↳"}</span><span><small>Ao terminar</small>${renderPackageInline(frame.continuation)}</span></p></li>`).join("")}</ol><p>Base: primeira chamada</p></figure></div>`;
  },
  accessibleText(data) {
