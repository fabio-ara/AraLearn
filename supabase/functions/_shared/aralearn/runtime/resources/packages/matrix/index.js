...
function renderMatrix(values, name = "", delimiters = "brackets") {
  ...
  return `<math class="runtime-matrix-item" display="block" aria-label="${escapePackageAttribute(accessibleName)}"><mrow>${name ? `<mi class="runtime-matrix-name">${escapePackageHtml(name)}</mi><mo>=</mo>` : ""}<mo class="runtime-matrix-delimiter is-left" fence="true" stretchy="true">${left}</mo><mtable class="runtime-matrix-grid">${body}</mtable><mo class="runtime-matrix-delimiter is-right" fence="true" stretchy="true">${right}</mo></mrow></math>`;
}
