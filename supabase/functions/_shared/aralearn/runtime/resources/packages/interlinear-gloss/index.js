...
export const interlinearGlossPackage = Object.freeze({
  ...
  editableTargets(data) { return [...data.units.flatMap((_, index) => [{ path: `units[${index}].form`, label: `Editar forma da unidade ${index + 1}` }, { path: `units[${index}].gloss`, label: `Editar glosa da unidade ${index + 1}` }]), { path: "translation", label: "Editar tradução livre" }, ...data.abbreviations.flatMap((_, index) => [{ path: `abbreviations[${index}].code`, label: `Editar abreviação ${index + 1}` }, { path: `abbreviations[${index}].meaning`, label: `Editar significado ${index + 1}` }])]; },
