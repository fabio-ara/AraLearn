...
export const planePackage = Object.freeze({
  ...
  render(data) {
    ...
    const keys = [[data.points?.length, "point", "Ponto"], [data.vectors?.length, "vector", "Vetor"],
      [data.paths?.length, "region", "Região ou trajetória"]];
    const objectKey = `<ul class="package-plane-object-key" aria-label="Convenções geométricas">${keys.filter(([present]) => present)
