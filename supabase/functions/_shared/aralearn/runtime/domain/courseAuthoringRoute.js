export const COURSE_AUTHORING_SECTIONS = Object.freeze([
  "content", "planning", "parameters", "sources", "audio", "review", "research", "people"
]);
...
const TARGET_DEFINITIONS = Object.freeze([
  ...
  Object.freeze({ option: "explanationId", query: "explanationId", kind: "microsequence_explanation" }),
...
const BUILD_OPTION_FIELDS = new Set([
  ...
  "studyUnitId", "explanationId", "annotationId", "sourceId", "anchorId", "unassigned", "revision"
]);
...
function targetAllowedForSection(target, section) {
  ...
  if (target.kind === "microsequence_explanation") return section === "content";
...
export function buildCourseAuthoringRoute(courseId, options = {}) {
  ...
  if (options.revision != null && (!Number.isSafeInteger(options.revision) || options.revision < 1 || section !== "content")) {
    throw new TypeError("A revisão de leitura é inválida.");
  }
...
export function parseCourseAuthoringRoute(hashValue) {
  ...
  let revision = null;
  if (rawParameters.at(-1)?.startsWith("revision=")) {
    const match = /^revision=([1-9][0-9]*)$/u.exec(rawParameters.pop());
    revision = Number(match?.[1]);
    if (section !== "content" || !Number.isSafeInteger(revision) || revision < 1) return null;
  }
