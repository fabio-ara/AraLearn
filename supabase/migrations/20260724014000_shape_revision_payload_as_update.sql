...
-- A normalização dos campos varia entre inserção e atualização. Na revisão
-- parcial, um campo ausente deve continuar ausente depois dessa normalização;
-- somente null explícito pode limpar uma coluna já existente.
...
begin
  ...
  v_payload := private.shape_store_payload(
    p_store_name,
    private.jsonb_to_snake(coalesce(p_payload, '{}'::jsonb)),
