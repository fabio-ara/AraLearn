-- Atualiza a descoberta de topologia de rede (hub/repetidor) no catálogo.
-- As referências de pacotes e seus schemas permanecem iguais. A escolha de
-- componentes conserva sua semântica; conteúdo e snapshots não são regravados.
begin;
...
do $preflight$
begin
  if public.get_aralearn_runtime_manifest()->>'schemaRevision' is distinct from '20260907222912'
    or private.course_component_catalog_v1()->>'version' is distinct from '1-5f0fd13d'
    or private.course_component_catalog_v1()->>'schemaFingerprint'
      is distinct from 'sha256:be520856aeb1c583daef1c0a39e5252633af92eede420eb03be47c1e79d060cc'
    or exists(select 1 from private.course_component_policy_assignments
      where policy->>'catalogVersion' is distinct from '1-5f0fd13d') then
    raise exception 'A revisão anterior do runtime ou catálogo divergiu.' using errcode='55000';
  end if;
end $preflight$;

create temporary table previous_network_catalog on commit drop as
  select private.course_component_catalog_v1()-'version' value;
create temporary table previous_network_policies on commit drop as
...
do $preservation$
begin
  if (select value from previous_network_catalog)
      is distinct from (private.course_component_catalog_v1()-'version') then
    raise exception 'A atualização não pode alterar referências ou schemas de pacotes.' using errcode='55000';
  end if;
  if exists(
    (select value from previous_network_policies except all
      select to_jsonb(assignment)#-'{policy,catalogVersion}' from private.course_component_policy_assignments assignment)
    union all
    (select to_jsonb(assignment)#-'{policy,catalogVersion}' from private.course_component_policy_assignments assignment
      except all select value from previous_network_policies)
  ) then
    raise exception 'A atualização não pode alterar escolhas, autoria ou datas das políticas.' using errcode='55000';
  end if;
end $preservation$;
...
begin
  manifest:=public.get_aralearn_runtime_manifest()||jsonb_build_object('schemaRevision','20260908000533');
