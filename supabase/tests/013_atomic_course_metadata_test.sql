...
select is(public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 ...
 '{"title":"Título novo","objective":"Objetivo novo"}'::jsonb)->>'idempotent','true','retry usa mesmo receipt');
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 '96000000-0000-4000-8000-000000000101',1,null,'[]','[]','[]','application',null,'metadata-only-01',
 '{"title":"Outro título","objective":"Objetivo novo"}'::jsonb)$t$,'23514','requestId reutilizado com composição incompatível.','hash inclui metadados');
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 '96000000-0000-4000-8000-000000000101',1,null,'[]','[]','[]','application',null,'metadata-stale-01',
 '{"title":"Outro título","objective":"Objetivo novo"}'::jsonb)$t$,'PT409','O Curso mudou; releia antes de salvar.','CAS rejeita versão antiga');
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000002',
 ...
 '{"title":"Outro título","objective":"Objetivo novo"}'::jsonb)$t$,'42501','Edição do Curso não autorizada.','estudante público não edita metadados');
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 '96000000-0000-4000-8000-000000000101',2,1,'[]','[]','[]','application','manual','metadata-focal-01',
 '{"title":"Outro título","objective":"Objetivo novo"}'::jsonb)$t$,'22023','Metadados não pertencem à edição focal de unidade.','metadados não contornam escopo focal');
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 '96000000-0000-4000-8000-000000000101',2,null,'[]','[]','[]','application',null,'metadata-shape-01',
 '{"title":"Outro título","objective":"Objetivo novo","ownerId":"outro"}'::jsonb)$t$,'22023','Metadados do curso inválidos.','nenhuma autoridade incidental no patch');
select is(public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 '96000000-0000-4000-8000-000000000101',2,null,'[]','[]','[]','application',null,'metadata-noop-01',
 '{"title":"Título novo","objective":"Objetivo novo"}'::jsonb)->>'revision','2','metadados iguais não alteram revisão');
select is(public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 ...
 '[]','[]','application',null,'metadata-rows-01','{"title":"Título combinado","objective":"Objetivo combinado"}'::jsonb)->>'revision','3','metadados e linhas usam uma revisão');
...
select throws_ok($t$select public.commit_course_composition_for_actor_v1('96000000-0000-4000-8000-000000000001',
 ...
 '[]','[]','application',null,'metadata-invalid-01','{"title":"Não persistir","objective":"Não persistir"}'::jsonb)$t$,
...
insert into auth.sessions(id,user_id,created_at,updated_at) values('96000000-0000-4000-8000-000000000901','96000000-0000-4000-8000-000000000001',now(),now());
...
select set_config('request.jwt.claim.sub','',true);
set local role anon;
select is(public.get_course_study_citations_v1('96000000-0000-4000-8000-000000000101',6,'u')->'citations'->0->'anchors'->0->>'humanLocator','p. 2','visitante recebe localização legível');
select ok(not (public.get_course_study_citations_v1('96000000-0000-4000-8000-000000000101',6,'u')->'citations'->0->'anchors'->0 ? 'verificationExcerpt'),'projeção pública não contém campo de verificação privado');
select is(public.get_course_study_citations_v1('96000000-0000-4000-8000-000000000101',6,'u')->'citations'->0->'attachments',
