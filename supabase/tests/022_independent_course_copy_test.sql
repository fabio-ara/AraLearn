...
('30600000-0000-4000-8000-000000000101','study_unit','u','microsequence','s',1,'{"title":"Unidade anterior","role":"theory","content":[{"id":"p","package":"aralearn.resource.paragraph","version":"1.0.0","data":{"text":"Uma ligação une dois pontos."}}],"response":null,"feedback":[],"topics":[]}');
...
-- Declaração sintética por unidade: a cópia deve preservar conteúdo e retirar a marca da origem.
select is(public.set_course_content_review_for_actor_v1(pg_temp.copy_owner(),pg_temp.copy_source(),'study_unit','u',
 private.course_content_basis_hash_v1(pg_temp.copy_source(),'study_unit','u'),true,'copy306-review-unit')#>>'{contentReview,state}',
 'current','Origem recebe declaração expressa de revisão da unidade salva');
...
select ok(not exists(select 1 from private.course_entities where course_id=pg_temp.copy_target() and entity_type in('microsequence','study_unit') and content_review is distinct from '{}'::jsonb),'Bases e unidades copiadas exigem revisão própria');
