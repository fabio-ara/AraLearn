...
export function renderSourceBibliographyForm(state) {
  ...
  return '<form class="course-source-form" data-source-form="source">' +
    `<h3>${state.sourceEditor.source ? "Editar referência" : "Documento ou link"}</h3>` +
    ...
    (!state.sourceEditor.source && state.canUploadPdf ? '<label class="course-source-new-pdf">Documento neste dispositivo' +
      '<span class="course-source-new-pdf-picker">' + renderUiIcon("upload", "course-authoring-button-icon") +
      `<span>${state.sourceEditor.pdfFile ? escape(state.sourceEditor.pdfFile.name) : "Escolher documento"}</span>` +
      `<input type="file" accept="application/pdf,.pdf" aria-label="Documento neste dispositivo" data-source-new-pdf-input${state.busy ? " disabled" : ""}></span></label>` : "") +
