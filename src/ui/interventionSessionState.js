...
export function normalizeInterventionSessionEntry(entry = {}, { reference = {}, baseVersionId = "" } = {}) {
  ...
  const feedbackText = text(entry?.feedbackText) || nextPromptDraft || text(entry?.message);
  return {
    ...normalizedReference,
    ...
    recommendedOperationMode: RECOMMENDED_OPERATION_MODES.has(text(entry?.recommendedOperationMode))
      ...
      : "",
    recommendedInterventionType: text(entry?.recommendedInterventionType),
...
export function interventionSessionNeedsIteration(session = {}) {
  return ["needs_retry", "needs_continue_here", "needs_support_microsequence", "needs_new_microsequence"].includes(
    text(session?.status)
  ) && !session?.stale;
}
