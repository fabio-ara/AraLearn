...
function resolveTarget(card, targetId) {
  ...
  if (requested.startsWith("body:")) {
    ...
    return index < 0 ? null : { value: card.blocks[index], collection: "blocks", index };
  }
  if (requested.startsWith("after:") && requested !== "after:text") {
    ...
    return index < 0 ? null : { value: card.afterBlocks[index], collection: "afterBlocks", index };
  }
  return { value: card, collection: "card", index: -1 };
}
...
export function buildManualCardEditModel(card = {}, targetId = "card") {
  ...
  return {
    ...
    fields,
    ...(Array.isArray(target.options) ? choiceModel(target) : {}),
    ...(Array.isArray(target.columns) && Array.isArray(target.rows) ? tableModel(target) : {})
  };
}

function applyFields(target, values) {
  SIMPLE_TEXT_FIELDS.forEach(([key]) => {
    if (Object.hasOwn(values, key) && Object.hasOwn(target, key)) {
      target[key] = String(values[key] ?? "");
    }
  });
  if (Array.isArray(target.options) && Array.isArray(values.optionValues)) {
  ...
  if (Array.isArray(target.columns) && Array.isArray(values.columns)) {
    target.columns = values.columns.map((value) => String(value ?? ""));
  }
  if (Array.isArray(target.rows) && Array.isArray(values.rows)) {
...
export function applyManualCardEdit(card = {}, targetId = "card", values = {}) {
  ...
  applyFields(resolved.value, values);
