...
import { normalizeEditorialOrigin, normalizeEditorialInterventions } from "../domain/courseEditorialProvenance.js";
import { normalizeCourseContentReviewState } from "../domain/courseContentReview.js";
import {
...
import { CourseAuthoringObservationQueue, authoringObservationQuery, orderAuthoringObservations,
  isPendingAuthoringObservation, observationTargetCatalog, observationDecisionSelection, filterAuthoringObservations, loadObservationComparison } from "./courseAuthoringObservationQueue.js";
...
const MAX_ANNOTATION_PAGES_PER_TARGET = 512;
...
function normalizeAnalysisIdea(value, label) {
  ...
  const description = value.description == null || value.description === ""
    ? null
    : requiredText(value.description, `A descrição de ${label.toLowerCase()}`, 4_000);
  return Object.freeze({
    name: requiredText(value.name, `O nome de ${label.toLowerCase()}`, 2_000),
    description
  });
}
...
function normalizeAuthorship(value) {
  exactRecord(
    value,
    ["createdOrigin", "lastRevisionOrigin", "design", ...(Object.hasOwn(value || {}, "interventions") ? ["interventions"] : [])],
    "O estado autoral da unidade é inválido."
  );
  const createdOrigin = normalizeEditorialOrigin(value.createdOrigin);
  const lastRevisionOrigin = normalizeEditorialOrigin(value.lastRevisionOrigin);
  ...
  if (application) {
    exactRecord(
      application,
      ["mode", "componentRefs", "analysisIdeas", ...(Object.hasOwn(application, "practiceEvidence") ? ["practiceEvidence"] : [])],
    ...
    if (application.practiceEvidence != null && (!Array.isArray(application.practiceEvidence) || application.practiceEvidence.length > 128)) {
      throw new TypeError("A evidência da prática é inválida.");
    }
    application.practiceEvidence = application.practiceEvidence == null ? null :
      application.practiceEvidence.map(item => normalizeAnalysisIdea(item, "Uma evidência da prática"));
  }
  return Object.freeze({
    createdOrigin,
    lastRevisionOrigin,
    ...(value.interventions ? { interventions: normalizeEditorialInterventions(value.interventions) } : {}),
...
export function inspectionRequestFromTarget(target) {
  ...
  if (target.kind === "microsequence_explanation") return Object.freeze({
    scope: normalizeScope({ kind: "didactic_microsequence", id: target.id }),
...
function renderManualModeActions(item, state, editing, observationCount) {
  ...
  return '<nav class="course-inspection-mode-actions" role="group" aria-label="Ações da unidade de estudo">' +
    ...
    ` data-inspection-control-key="observations:${id}" aria-label="Observações autorais do curso${observationCount === null ? ", contagem ainda não disponível" : `, ${Number(observationCount)} pendentes`}"` +
...
function renderInspectionBibliography(item, state) {
  ...
  if (entry?.status === "ready") {
    return entry.snapshot.value.citations.length
      ? renderStudyCitations({ open: true, value: entry.snapshot.value, courseId: state.courseId, studyUnit: item.studyUnit,
        formattedReferences: entry.snapshot.formattedReferences, heading: "Referências desta unidade", contextId: "unit" })
      : '<p class="course-authoring-notice" role="status">Nenhuma referência preparada para esta unidade.</p>';
  }
  if (entry?.status === "error") {
    return '<p class="course-authoring-notice is-error" role="status">Não foi possível carregar as referências.</p>' +
...
function renderStudyUnit(
  ...
  const runtime = renderPackageStudyUnitBlocksWithDock(item.studyUnit, {
    ...
    ...practicePreview?.renderOptions(item.studyUnit, `inspection:${item.studyUnit.id}`, { editing }),
  ...
  return `<li class="course-inspection-item${selected ? " is-selected" : ""}" data-inspection-study-unit="${escapeHtml(item.studyUnit.id)}"` +
    ...
    '<div class="course-inspection-item-detail-panel">' +
    renderCourseInspectionMetadata(item, state, observationCount) +
    "</div></details></header>" +
    ...
    (state.canReviewContent ? `<button type="button" data-inspection-open-explanation data-study-unit-id="${escapeHtml(item.studyUnit.id)}"` +
      ` data-inspection-control-key="explanation:${escapeHtml(item.studyUnit.id)}" aria-label="Abrir explicação" title="Abrir explicação"${state.manualStudyUnitId ? " disabled" : ""}>` +
    ...
    renderAuthorshipState(item, state) +
...
function renderAuthorshipState(item, state) {
  const review = item.contentReview?.state || 'unknown';
  const icon = ({ current: 'ready-state', stale: 'rotate', unknown: 'cloud-alert' })[review] || 'draft-state';
  const label = contentReviewLabel(item);
  return '<div class="course-inspection-authorship course-authoring-compact-actions" aria-label="Revisão autoral">' +
    (state.canReviewContent
      ? `<button type="button" data-inspection-review-unit data-study-unit-id="${escapeHtml(item.studyUnit.id)}"` +
        ` data-inspection-review-state="${escapeHtml(review)}" aria-label="${escapeHtml(label)}" title="${escapeHtml(label)}"` +
        ` data-inspection-control-key="review-state:${escapeHtml(item.studyUnit.id)}"${state.manualStudyUnitId ? ' disabled' : ''}>` +
        renderUiIcon(icon, 'course-authoring-button-icon') + '</button>'
      : `<span data-inspection-review-state="${escapeHtml(review)}" role="img" aria-label="${escapeHtml(label)}" title="${escapeHtml(label)}">` +
        renderUiIcon(icon, 'course-authoring-button-icon') + '</span>') + '</div>';
}
...
function renderInspectionObservationSheet(state) {
  ...
  return renderStudyUnitObservationSheet({
    ...
    pending: Boolean(state.pendingObservationMutation),
    recoveryExpired: state.observationRecoveryExpired,
    loading: state.observationLoading,
    title: "Observações do curso",
    ariaLabel: "Central de observações autorais",
    listLabel: "Observações autorais pendentes",
    emptyLabel: "Nenhuma observação autoral pendente neste contexto.",
    showContributor: false,
    availableTargets: state.observationTargetCatalog,
    filters: state.observationFilters,
    selectedObservationIds: state.observationSelectedIds,
    selectedTargetKeysByAnnotation: state.observationDecisionTargets,
    cancelReason: state.observationCancelReason,
    selectedTargets: state.observationSelectedTargets || state.observationTargetIds.map(id => ({ kind: "study_unit", id })),
    ...
    contextMessage: state.observationMessage || (batch
      ? "Uma observação será vinculada aos alvos selecionados."
      : ""),
...
function renderTemporarySelection(state) {
  ...
  const expanded = count > 0 && state.selectionComposerOpen && !state.observationSheetOpen;
  const targets = [...state.selectedStudyUnitIds].map(id =>
    state.selectedStudyUnitTitles[id] || state.items.find(item => item.studyUnit.id === id)?.studyUnit.title || "Unidade selecionada");
  return `<aside class="course-inspection-selection${count ? ' has-targets' : ''}" data-inspection-selection-bar` +
    ' aria-label="Seleção temporária de unidades de estudo">' +
    '<div class="course-inspection-selection-heading">' +
    (count ? '<details class="course-inspection-selection-targets"><summary title="Unidades selecionadas">' : '<span>') +
    `<span role="status">${count > 0
      ? `${count} ${count === 1 ? "unidade selecionada" : "unidades selecionadas"}`
      : "Selecione uma ou mais unidades para comentar"}</span>` +
    (count ? '</summary><ol>' + targets.map(title => `<li>${escapeHtml(title)}</li>`).join('') + '</ol></details>' : '</span>') +
    (count ? '<div class="course-authoring-compact-actions">' + `<button type="button" class="course-authoring-icon-action"` +
    ' data-inspection-selection-action="observe-selected"' +
    ' data-inspection-control-key="selection:observe"' +
    ` aria-expanded="${expanded}" aria-label="${expanded ? 'Recolher caixa de observação' : 'Escrever observação nas unidades selecionadas'}"` +
    ` title="${expanded ? 'Recolher caixa de observação' : 'Escrever observação'}">` +
    ...
    `${renderUiIcon("remove-state", "course-authoring-button-icon")}</button></div>` : '') + '</div>' +
    (expanded ? renderStudyUnitObservationComposer({
      draft: state.observationDraft,
      error: state.observationError,
      saving: state.observationSaving,
      compact: true,
      selectionComposer: true
    }) +
      (state.observationMessage ? `<p class="course-inspection-selection-status" role="status">${escapeHtml(state.observationMessage)}</p>` : '') : '') +
    '</aside>';
}
...
function renderSelectionPagination(state, direction) {
  ...
  return '<div class="course-inspection-selection-pagination">' +
    ...
    ` data-inspection-control-key="selection:${direction}" aria-label="${label}" title="${label}"` +
    `${state.loadingDirection ? ' disabled aria-disabled="true"' : ""}>` +
    `${renderUiIcon(previous ? "arrow-left" : "arrow-right", "course-authoring-button-icon")}</button></div>`;
}

function renderSequence(state, practicePreview) {
  ...
  const notice = state.initialFailure && state.items.length > 0
      ...
      : state.hydrationFailure
          ...
          : state.intendedRevision != null && state.intendedRevision !== state.pinnedRevision
              ? '<p class="course-authoring-notice" role="status">O conteúdo mudou desde este link. Você está lendo a versão atual.</p>'
          : "";
  ...
  if (state.initialLoading && state.items.length === 0) {
    ...
    body = '<section class="course-authoring-state is-error" role="alert">' +
      ...
      '<p>O conteúdo solicitado foi removido ou não está disponível neste curso.</p>' +
    ...
    body = '<section class="course-authoring-state is-empty" role="status">' +
      ...
      (state.emptyMicrosequences.length && state.canReviewContent
        ? '<p>As explicações podem ser consultadas e revisadas antes das unidades.</p>' +
          '<ul class="course-inspection-empty-explanations" aria-label="Explicações das microssequências">' + state.emptyMicrosequences.map(microsequence =>
            `<li class="course-inspection-explanation-entry"><span class="course-inspection-explanation-title">${escapeHtml(microsequence.title)}</span><button type="button" class="course-authoring-icon-action" data-inspection-open-explanation` +
            ` data-microsequence-id="${escapeHtml(microsequence.id)}" data-inspection-control-key="empty:explanation:${escapeHtml(microsequence.id)}"` +
            ` aria-label="Explicação de ${escapeHtml(microsequence.title)}" title="Abrir explicação">${renderUiIcon("study", "course-authoring-button-icon")}</button></li>`).join('') + '</ul>'
  ...
  return `<section class="course-authoring-section course-authoring-inspection${state.multipleView && state.selectedStudyUnitIds.size ? ' has-selection-dock' : ''}"` +
...
function annotationTargetQuery() {
  return authoringObservationQuery();
}
...
export function createCourseInspectionSequence({
  ...
  const state = {
    ...
    canReviewContent: course.ownership === "owned" && course.canEdit === true &&
      typeof controller.getContentReview === "function" && typeof controller.setContentReview === "function" && typeof controller.exportCourseAuthoring === "function",
    ...
    hydrationFailure: "",
    observationCounts: {},
    observationTargetCatalog: [],
    observationSelectedTargets: null,
    observationFilters: {},
    observationSelectedIds: [],
    observationDecisionTargets: {},
    observationCancelReason: "withdrawal",
    observationRecoveryExpired: false,
    citations: new Map(),
    emptyMicrosequences: [],
    multipleView: false,
    modeBlock: null,
    ...
    selectedStudyUnitTitles: {},
    observationSelectionDraft: false,
    ...
    observationEditingSnapshot: null,
  ...
  function captureAnchor() {
    ...
    return {
      ...
      containerScrollTop: scrollTarget && "scrollTop" in scrollTarget ? scrollTarget.scrollTop : null,
  ...
  function restoreAnchor(snapshot, { initial = false } = {}) {
    ...
    if (!id) {
      if (Number.isFinite(snapshot?.containerScrollTop) && scrollTarget && "scrollTop" in scrollTarget) scrollTarget.scrollTop = snapshot.containerScrollTop;
      restoreControlState(snapshot); return;
    }
  ...
  function deactivateResponses() {
    root.querySelectorAll?.(
      ...
      const itemId = container.closest?.('[data-inspection-study-unit]')?.dataset.inspectionStudyUnit;
      if (state.items.find(item => item.studyUnit.id === itemId)?.studyUnit.response?.package === 'aralearn.response.gap') return;
  ...
  async function hydrate(epoch) {
    try {
      ...
      deactivateResponses();
      root.querySelectorAll?.('[data-inspection-study-unit]').forEach(scope => {
        const item = state.items.find(value => value.studyUnit.id === scope.dataset.inspectionStudyUnit);
        if (!item) return;
        const editing = state.manualStudyUnitId === item.studyUnit.id && !state.manualPreview;
        const unit = state.manualStudyUnitId === item.studyUnit.id && state.manualPreview || item.studyUnit;
  ...
  function render({
    ...
    root.innerHTML = renderSequence(state, practicePreview);
  ...
  function mergePage(page, direction) {
    ...
    page.items.forEach((item) => {
      ...
      state.observationCounts[item.studyUnit.id] = item.pendingAuthoringObservationCount;
    });
  ...
  async function loadTargetObservations() {
    const query = annotationTargetQuery();
    const items = [];
    ...
    for (let index = 0; index < MAX_ANNOTATION_PAGES_PER_TARGET; index += 1) {
      ...
      if (!page.hasMore) {
        if (items.length !== summary.matchingTotal) throw new Error("A central está incompleta; atualize antes de decidir.");
        return {
          items: orderAuthoringObservations(items.filter(isPendingAuthoringObservation)),
  ...
  async function openObservations(studyUnitId) {
    ...
    if ((state.observationSelectionDraft && (hasObservationDraft() || state.pendingBatchObservation)) ||
        hasObservationDraft() && state.observationDraftStudyUnitId &&
    ...
    if (!hasObservationDraft()) {
      ...
      state.observationEditingId = null; state.observationEditingSnapshot = null;
    }
    state.observationDraftStudyUnitId = studyUnitId;
    ...
    render({ anchor });
    try {
      const recovery = new CourseAuthoringObservationQueue({ controller, courseId: state.courseId,
        targetKind: "study_unit", targetId: studyUnitId, expectedRevision: state.pinnedRevision });
      await recovery.restorePending();
      if (state.destroyed || epoch !== observationEpoch) return false;
      if (recovery.pending) {
        state.pendingObservationMutation = { request: recovery.pending, draft: null };
        state.observationDraft = { category: recovery.pending.command.category, rawText: recovery.pending.command.rawText };
        state.observationEditingId = recovery.pending.command.type === "revise_anchored_annotation" ? recovery.pending.command.annotationId : null;
        state.observationMessage = "Confirme o resultado do envio pendente antes de adicionar outra observação.";
      }
  ...
  async function openBatchObservations() {
    const targetIds = [...state.selectedStudyUnitIds];
    if (targetIds.length < 1 || state.observationSaving) return false;
    ...
    state.observationSheetOpen = true;
    state.observationSelectionDraft = true;
  ...
  async function mutateBatchObservation(targetIds, rawText, category) {
    if (typeof controller.mutateCourseAnchoredAnnotations !== "function" ||
        !Array.isArray(targetIds) || targetIds.length < 1) return false;
    ...
    state.pendingBatchObservation ||= {
      ...
      requests: targetIds.slice(0, 1).map((targetId) => {
        const command = normalizeCourseAnchoredAnnotationCommand({
          ...
          targets: state.observationSelectedTargets || targetIds.map(id => ({ kind: "study_unit", id })),
          rawText,
          category,
          briefSummary: null,
    ...
    try {
      for (const entry of pending.requests) {
        ...
        const recovery = new CourseAuthoringObservationQueue({ controller, courseId: state.courseId,
          targetKind: "study_unit", targetId: entry.targetId, expectedRevision: state.pinnedRevision });
        await recovery.restorePending();
        if (recovery.pending && recovery.pending.requestId !== entry.request.requestId) {
          throw Object.assign(new Error("Esta unidade tem outro envio pendente. Abra sua fila e confirme o resultado antes de enviar este lote."), { ambiguous: true });
        }
        const reconciled = recovery.pending ? await recovery.reconcilePending() : false;
        if (!reconciled) {
        await recovery.persistPending(entry.request);
        ...
        await recovery.persistPending(null);
        }
      ...
      state.observationDraftStudyUnitId = targetIds[0];
      state.observationMessage =
        targetIds.length === 1 ? "Observação registrada nesta unidade." :
          `Observação registrada nas ${targetIds.length} unidades selecionadas.`;
      return true;
    } catch (error) {
  ...
  async function mutateObservation(command, successMessage, { operationDraft = command } = {}) {
    ...
    if (state.pendingObservationMutation?.draft && !pendingObservationMatches(state.pendingObservationMutation, draft)) {
      state.observationError = "Confirme o envio pendente antes de alterar a observação.";
      render(); return false;
    }
    if (!state.pendingObservationMutation) {
      state.pendingObservationMutation = null;
    }
    const expectedCourseRevision = ["create_anchored_annotation", "retarget_anchored_annotation"].includes(normalized.type)
    ...
    state.restoreObservationCloseFocus = true;
    render();
    const targetId = state.observationStudyUnitId;
    const mutationEpoch = observationEpoch;
    const stillCurrent = () => !state.destroyed && observationEpoch === mutationEpoch && state.observationStudyUnitId === targetId;
    const recovery = new CourseAuthoringObservationQueue({ controller, courseId: state.courseId,
      targetKind: "study_unit", targetId, expectedRevision: state.pinnedRevision });
    let mutationConfirmed = false;
    let writeStarted = false; let writeReturned = false;
    try {
      await recovery.restorePending();
      if (recovery.pending && recovery.pending.requestId !== request.requestId) {
        throw Object.assign(new Error("Há outro envio pendente nesta fila. Reabra o objeto para conferir o resultado."), { ambiguous: true });
      }
      if (recovery.pending && await recovery.reconcilePending()) {
        mutationConfirmed = true;
      } else {
        await recovery.persistPending(request);
        writeStarted = true;
        const response = await controller.mutateCourseAnchoredAnnotations(structuredClone(request));
        writeReturned = true;
        const change = normalizeCourseAnchoredAnnotationChange(response);
      ...
      state.observationDraftStudyUnitId = state.selectedStudyUnitId;
      state.observationEditingId = null;
      state.observationEditingSnapshot = null;
      const observations = await loadTargetObservations(studyUnitId);
      if (!stillCurrent()) return true;
      state.observationItems = observations.items;
      state.observationCollectionSummary = observations;
      ...
      const ambiguous = !writeStarted || writeReturned || ambiguousObservationFailure(error);
      if (!ambiguous) { state.pendingObservationMutation = null; await recovery.persistPending(null); }
  ...
  async function loadInitial({
    ...
    try {
      ...
      if (anchorStudyUnitId && !page.items.some(item => item.studyUnit.id === anchorStudyUnitId)) {
      ...
      if (state.items.length === 0) {
        try {
          ...
          let partMembers = null;
          if (state.scope.kind === "authoring_part") {
            const plan = await controller.loadAuthoringPlan(state.courseId);
            if (state.destroyed || epoch !== requestEpoch) return false;
            if (plan.courseId !== state.courseId || plan.courseRevision !== state.pinnedRevision) {
              throw new Error("O planejamento mudou durante a leitura.");
            }
            partMembers = new Set(plan.plan.parts.find(part => part.id === scopeId)?.microsequences.map(item => item.id) || []);
          }
          state.emptyMicrosequences = index.filter(entry => entry.kind === "didactic_microsequence" &&
            (partMembers ? partMembers.has(entry.id) : state.scope.kind === "course" || entry.entityPath.includes(scopeId)));
        } catch {
          state.emptyScopeContext = null;
        }
      }
      ...
      if (!state.destroyed && epoch === requestEpoch) {
        ...
        render({
          ...
          initial: true,
          restorePosition: hasRequestedPosition || Boolean(returnAnchor)
        });
  ...
  function beginManualEdit(
    ...
    if (!item) return false;
    if (targetId === 'study_unit' && item.studyUnit.response?.package === 'aralearn.response.gap') {
      targetId = `response:${item.studyUnit.response.id}`;
    }
  ...
  async function handleClick(event) {
    ...
    if (pendingAction && state.modeBlock) {
      ...
      state.modeBlock = null;
      if (kind === "observation") {
        if (state.observationSelectionDraft || state.pendingBatchObservation || state.observationTargetIds.length > 1 ||
    ...
    if (selectionAction) {
      ...
      if (action === "toggle-unit" && state.multipleView) {
        ...
        if (state.selectedStudyUnitIds.has(studyUnitId)) {
          ...
          delete state.selectedStudyUnitTitles[studyUnitId];
        } else if (state.selectedStudyUnitIds.size < 64) {
          state.selectedStudyUnitIds.add(studyUnitId);
          state.selectedStudyUnitTitles[studyUnitId] = state.items.find(item => item.studyUnit.id === studyUnitId).studyUnit.title;
        } else {
        ...
        state.modeBlock = null;
        state.observationTargetIds = [...state.selectedStudyUnitIds];
        state.observationStudyUnitId = state.observationTargetIds[0] || null;
        state.observationDraftStudyUnitId = state.observationStudyUnitId;
        state.observationSelectionDraft = state.observationTargetIds.length > 0;
        state.selectionComposerOpen = state.observationSelectionDraft;
        state.observationError = "";
        state.observationMessage = "";
        render({ anchor });
        return true;
      }
      if (action === "observe-selected") {
        if (state.selectionComposerOpen) {
          state.selectionComposerOpen = false;
          render({ anchor: captureAnchor() });
          return true;
        }
        return openBatchObservations();
      }
    ...
    if (observationAction) {
      ...
      if (action === "compare") {
        const item = state.observationItems.find(value => value.annotationId === annotationId);
        const panel = observationAction.closest('[data-observation-comparison]'); observationAction.disabled = true;
        try {
          const target = await loadObservationComparison(controller, state.courseId, item, observationAction.dataset.observationTargetKey);
          if (panel.isConnected) panel.innerHTML = renderObservationComparison(target);
        } catch (error) { observationAction.disabled = false; observationAction.textContent = publicErrorMessage(error, 'Não foi possível carregar. Tente novamente.'); }
        return true;
      }
      if (action === "refresh") return openObservations(state.observationStudyUnitId);
      if (action === "retry" || action === "abandon-expired") {
        const queue = new CourseAuthoringObservationQueue({ controller, courseId: state.courseId, targetKind: "study_unit",
          targetId: state.observationStudyUnitId, expectedRevision: state.pinnedRevision });
        try {
          await queue.restorePending();
          if (action === 'abandon-expired') await queue.abandonExpiredAttempt(); else await queue.resumeDecisions();
          state.pendingObservationMutation = null; state.observationRecoveryExpired = false; return openObservations(state.observationStudyUnitId);
        } catch (error) { state.observationRecoveryExpired = error.code === 'observation_recovery_expired'; state.observationError = publicErrorMessage(error, "A mesma tentativa continua pendente."); render(); return true; }
      }
      if (/^(approve|cancel)(-|$)/u.test(action) && action !== "cancel-edit") {
        if (state.observationSaving) return true;
        const selectedIds = new Set([...root.querySelectorAll("[data-observation-select]:checked")].map(node => node.dataset.observationId));
        const presented = filterAuthoringObservations(state.observationItems, state.observationFilters).filter(value => value.targetSetVersion);
        const items = annotationId ? presented.filter(item => item.annotationId === annotationId)
          : action.endsWith("-all") ? presented : presented.filter(item => selectedIds.has(item.annotationId));
        if (!items.length) {
          state.observationError = action.endsWith("-selected")
            ? "Selecione ao menos uma observação elegível para decidir."
            : "Não há observações elegíveis para decidir neste contexto.";
          render();
          return true;
        }
        const targetKeysByAnnotation = observationDecisionSelection(root);
        const decision = action.startsWith("approve") ? "approve" : "cancel";
        const reason = decision === "cancel" ? root.querySelector("[data-observation-cancel-reason]").value : null;
        const queue = new CourseAuthoringObservationQueue({ controller, courseId: state.courseId, targetKind: "study_unit",
          targetId: state.observationStudyUnitId, expectedRevision: state.pinnedRevision });
        state.observationSaving = true; state.observationError = ""; render();
        try {
          await queue.restorePending(); await queue.decide(items, { decision, reason, targetKeysByAnnotation });
          const observations = await loadTargetObservations(); state.observationItems = observations.items;
          state.observationCollectionSummary = observations; state.observationMessage = "Decisão salva.";
        } catch (error) { state.observationError = publicErrorMessage(error, "A decisão não pôde ser confirmada; a pendência foi preservada."); }
        finally { state.observationSaving = false; render(); }
        return true;
      }
      if (action === "retarget") {
        const item = state.observationItems.find(value => value.annotationId === annotationId);
        const select = [...root.querySelectorAll("[data-observation-retarget]")].find(node => node.dataset.observationRetarget === annotationId);
        const targets = [...(select?.selectedOptions || [])].map(option => {
          const split = option.value.indexOf(":"); return { kind: option.value.slice(0, split), id: option.value.slice(split + 1) };
        });
        return mutateObservation({ type: "retarget_anchored_annotation", annotationId, expectedAnnotationVersion: item.annotationVersion,
          expectedTargetSetVersion: item.targetSetVersion, targets }, "Alvos atualizados.");
      }
      if (action === "close") {
        if (state.pendingBatchObservation || state.pendingObservationMutation || state.observationSaving) {
        ...
        const preserveCreationDraft = hasObservationDraft();
        state.observationSheetOpen = false;
        state.observationStudyUnitId = null;
        ...
        if (!preserveCreationDraft) { state.observationEditingId = null; state.observationEditingSnapshot = null; }
        state.observationMessage = "";
        render({ anchor });
      } else if (action === "edit") {
        ...
        if (item?.capabilities?.canRevise) {
          ...
          state.observationEditingSnapshot = structuredClone(item);
  ...
  function handleChange(event) {
    if (event.target.matches?.('[data-observation-filter]')) {
      state.observationFilters[event.target.dataset.observationFilter] = event.target.value; render(); return;
    }
    if (event.target.matches?.('[data-observation-select]')) {
      state.observationSelectedIds = [...root.querySelectorAll('[data-observation-select]:checked')].map(node => node.dataset.observationId);
      syncAuthoringObservationDecisions(root, {
        items: state.observationItems, filters: state.observationFilters,
        selectedObservationIds: state.observationSelectedIds, saving: state.observationSaving
      });
      return;
    }
    if (event.target.matches?.('[data-observation-target-select]')) {
      state.observationDecisionTargets = { ...state.observationDecisionTargets, ...observationDecisionSelection(root) }; return;
    }
    if (event.target.matches?.('[data-observation-cancel-reason]')) { state.observationCancelReason = event.target.value; return; }
    if (event.target.matches?.("[data-observation-new-targets]")) {
      state.observationSelectedTargets = [...event.target.selectedOptions].map(option => {
        const split = option.value.indexOf(":"); return { kind: option.value.slice(0, split), id: option.value.slice(split + 1) };
      });
      return;
    }
  ...
  function handleSubmit(event) {
    ...
    if (!state.observationEditingId && (state.observationSelectionDraft && !state.observationSheetOpen || state.observationTargetIds.length > 1)) {
    ...
    const editing = state.observationEditingSnapshot || state.observationItems.find(({ annotationId }) =>
    ...
    const operationDraft = editing ? {
      ...
      type: "create_anchored_annotation",
      target: state.observationSelectedTargets?.[0] || { kind: "study_unit", id: submittedStudyUnitId },
      ...(state.observationSelectedTargets ? { targets: state.observationSelectedTargets } : {}),
      rawText,
      category: state.observationDraft.category,
      briefSummary: null
    };
    const command = state.pendingObservationMutation && (state.pendingObservationMutation.draft === null || pendingObservationMatches(state.pendingObservationMutation, operationDraft))
  ...
  function hasPendingDraft() {
    ...
    return Boolean(
      contentReview.hasPendingDraft() || state.pendingObservationMutation || state.pendingBatchObservation ||
  ...
  return Object.freeze({
    ...
    async open() {
      ...
      if (restorePersistedPosition) {
        // A saved scope is only contextual. An explicit link identifies the object,
        // including when that object has moved to another part since the last visit.
        state.scope = routeTarget?.kind === "study_unit" && position.courseRevision !== state.pinnedRevision
          ? requested.scope : normalizeScope(position.scope);
        state.requestedAnchorStudyUnitId = canonicalId(
          position.studyUnitId,
          "A unidade de estudo da posição"
        );
      }
      const opened = await loadInitial({
        anchorStudyUnitId: state.requestedAnchorStudyUnitId,
        offset: restorePersistedPosition ? Number(position.offsetFromStickyTop || 0) : 0
      });
      if (opened && state.canReviewContent && (routeTarget?.kind === "microsequence_explanation" ||
          state.items.length === 0 && state.emptyMicrosequences.length > 0 &&
          ["authoring_part", "didactic_microsequence"].includes(routeTarget?.kind))) {
        const microsequenceId = routeTarget.kind === "microsequence_explanation"
          ? routeTarget.id : state.emptyMicrosequences[0].id;
        await contentReview.open({ courseId: state.courseId, microsequenceId,
          expectedRevision: state.pinnedRevision, intendedRevision,
          button: [...(root.querySelectorAll?.("[data-microsequence-id]") || [])]
            .find(node => node.dataset.microsequenceId === microsequenceId) || null });
      }
      return opened;
    },
