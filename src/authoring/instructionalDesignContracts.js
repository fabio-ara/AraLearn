...
function keyForPackage(value) {
  return `${normalizedText(value?.packageId)}@${normalizedText(value?.version)}`;
}
...
export function evaluateInstructionalDesignBundle(raw) {
  ...
  const analysisKey = `${analysis?.id}@${analysis?.version}`;
  const snapshotAnalysisKey = `${snapshot?.analysisRef?.id}@${snapshot?.analysisRef?.version}`;
  const manifestAnalysisKey = `${manifest?.analysisRef?.id}@${manifest?.analysisRef?.version}`;
  const snapshotKey = `${snapshot?.id}@${snapshot?.version}`;
  const manifestSnapshotKey = `${manifest?.effectiveSnapshotRef?.id}@${manifest?.effectiveSnapshotRef?.version}`;
  ...
  if (snapshot?.basedOnWorkspaceRevision !== analysis?.derivedFrom?.workspaceRevision) {
    errors.push("effectiveSnapshot.basedOnWorkspaceRevision diverge da revisão usada pela análise.");
  }
  ...
  if (Number.isInteger(manifest?.scopeEntityVersion)
    ...
    errors.push("materializationManifest.scopeEntityVersion antecede a versão analisada.");
  }
  ...
  const assignmentsByRef = new Map(
    assignments.map((assignment) => [`${assignment.id}@${assignment.version}`, assignment])
  );
  const assignmentSlots = new Set();
  assignments.forEach((assignment, index) => {
    const definitionKey = `${assignment?.definitionRef?.id}@${assignment?.definitionRef?.version}`;
  ...
  list(snapshot?.resolvedValues).forEach((resolved, index) => {
    const definitionKey = `${resolved?.definitionRef?.id}@${resolved?.definitionRef?.version}`;
    const definition = definitionsByRef.get(definitionKey);
    ...
    const assignmentKey = assignmentRef ? `${assignmentRef.id}@${assignmentRef.version}` : "";
  ...
  resourceSets.forEach((resourceSet, index) => {
    const resourceSetKey = `${resourceSet?.id}@${resourceSet?.version}`;
  ...
  const manifestResourceSetKeys = list(manifest?.resourceSetRefs).map(
    (resourceSetRef) => `${resourceSetRef?.id}@${resourceSetRef?.version}`
  );
  ...
  list(manifest?.resourceSelections).forEach((selection, index) => {
    ...
    const authorizerKey = `${selection?.authorizedByResourceSetRef?.id}@${selection?.authorizedByResourceSetRef?.version}`;
    ...
    if (selection?.fit === "substitute" && !list(selection?.limitations).length) {
