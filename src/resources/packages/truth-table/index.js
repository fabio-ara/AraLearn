...
export const truthTablePackage = Object.freeze({
  ...
  authoringContract: Object.freeze({
    ...
    example: Object.freeze({ prompt: "Observe a negação.", variables: ["P"], derivedColumns: ["¬P"], rows: [{ values: ["true"], results: ["false"] }, { values: ["false"], results: ["true"] }] })
  }),
  ...
  practiceTargets(data) { return [...data.variables.map((_, index) => ({ path: `variables[${index}]`, label: `Lacuna na variável ${index + 1}`, modes: ["gap", "typing"] })), ...data.derivedColumns.map((_, index) => ({ path: `derivedColumns[${index}]`, label: `Lacuna na fórmula ${index + 1}`, modes: ["gap", "typing"] }))]; }
});
