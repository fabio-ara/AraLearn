...
export const memoryLayoutPackage = Object.freeze({
  ...
  render(data) { const segments = orderedSegments(data); const direction = data.addressOrder === "ascending" ? "crescentes" : "decrescentes"; const topEdge = data.addressOrder === "ascending" ? "start" : "end"; const bottomEdge = data.addressOrder === "ascending" ? "end" : "start"; const arrow = "↓"; return `<div class="runtime-block package-memory-layout">${data.prompt ? renderPackageProse(data.prompt) : ""}<p class="package-memory-direction"><span>Endereços</span><strong>${direction}</strong><span aria-hidden="true">${arrow}</span></p><ol class="package-memory-stack" data-address-order="${data.addressOrder}">${segments.map((segment) => `<li><code>${renderPackageInline(displayAddress(data, segment, topEdge))}</code><div class="is-${segment.kind}"><strong>${renderPackageInline(segment.label)}</strong><span>${renderPackageInline(segment.description)}</span></div><code>${renderPackageInline(displayAddress(data, segment, bottomEdge))}</code></li>`).join("")}</ol></div>`; },
