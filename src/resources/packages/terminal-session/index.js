...
export const terminalSessionPackage = Object.freeze({
  ...
  render(data, options = {}) {
    return `<div class="runtime-block package-terminal-session">${renderPackageProse(data.prompt)}<figure><figcaption><strong>${renderPackageInline(data.environment)}</strong></figcaption>${data.initialContext ? `<p class="package-terminal-context"><span>Contexto inicial</span>${renderPackageInline(data.initialContext)}</p>` : ""}<ol aria-label="Interações da sessão">${data.interactions.map((interaction, index) => `<li data-terminal-interaction="${index + 1}"><header><strong aria-label="Interação ${index + 1}">${index + 1}</strong>${interaction.effect ? `<span class="package-terminal-effect">${renderPackageInline(interaction.effect)}</span>` : ""}</header><div class="package-terminal-input" role="group" aria-label="Entrada da interação ${index + 1}"><pre tabindex="0" aria-label="Entrada da interação ${index + 1}"><code>${interaction.prompt ? `<span class="package-terminal-prompt">${renderPackageCode(interaction.prompt)} </span>` : ""}${renderPackageCode(interaction.input)}</code></pre></div><details class="package-terminal-result"${options.manualEditing === true ? " open" : ""}><summary aria-label="Mostrar ou recolher resultado da interação ${index + 1}" title="Saídas e código de saída da interação ${index + 1}">${renderPackageActionIcon("down")}</summary><div class="package-terminal-streams">${streamBlock(interaction, index, "stdout", "Saída padrão")}${streamBlock(interaction, index, "stderr", "Erro padrão")}</div>${hasOwn(interaction, "exitCode") ? `<p class="package-terminal-exit${interaction.exitCode === 0 ? " is-success" : " is-error"}"><span>Código de saída</span><code>${interaction.exitCode}</code></p>` : ""}</details></li>`).join("")}</ol></figure></div>`;
  },
  accessibleText(data) {
