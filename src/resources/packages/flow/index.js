...
function flowEditableTargets(value, path = "structure", output = []) { if (Array.isArray(value)) { value.forEach((item, index) => flowEditableTargets(item, `${path}[${index}]`, output)); return output; } if (!value || typeof value !== "object") return output; Object.entries(value).forEach(([key, child]) => { const childPath = `${path}.${key}`; if (typeof child === "string" && !["id", "kind"].includes(key)) output.push({ path: childPath, label: `Editar ${key}` }); else if (child && typeof child === "object") flowEditableTargets(child, childPath, output); }); return output; }

export const flowPackage = Object.freeze({
  manifest: Object.freeze({ id: "aralearn.resource.flow", version: "1.0.0", label: "Fluxo", purpose: "Representar sequência, decisão, ramificação e repetição a partir de lógica declarativa.", slots: Object.freeze(["content", "feedback"]), cognitiveOperations: Object.freeze(["trace-control-flow", "decide", "recognize-loop", "predict-path"]), academic: academicProfile({ domains: ["algoritmos", "processos", "engenharia de software"], knowledgeObjects: ["fluxograma", "decisão", "ramificação", "repetição"], conventions: ["símbolos de início, processo e decisão distinguíveis", "ramos nomeados", "fluxo orientado"], appropriateWhen: ["o caminho depende de decisão ou repetição"], avoidWhen: ["há apenas ordem linear ou estados persistentes"], technologies: ["HTML semântico", "estrutura declarativa"], practiceModes: ["exposition", "gap", "typing", "selection", "ordering"] }), responseCompatibility: Object.freeze(["aralearn.response.gap", "aralearn.response.choice", "aralearn.response.ordering"]), limitations: Object.freeze(["Use sequence quando não houver ramificação.", "A geometria é derivada e nunca autoral." ]), accessibility: "A lógica é exposta também como árvore ordenada, independente do desenho." }),
