...
export const stateMachinePackage = Object.freeze({
  ...
  editableTargets(data) {
    const visibleEvents = new Set(data.transitions.map(({ event }) => event));
    return [...(data.prompt ? [{ path: "prompt", label: "Editar orientação" }] : []), ...data.states.map((_, index) => ({ path: `states[${index}].label`, label: `Editar estado ${index + 1}` })), ...data.events.flatMap((event, index) => visibleEvents.has(event.id) ? [{ path: `events[${index}].label`, label: `Editar evento ${index + 1}` }] : []), ...data.transitions.flatMap((transition, index) => [...(transition.guard ? [{ path: `transitions[${index}].guard`, label: `Editar guarda ${index + 1}` }] : []), ...(transition.action ? [{ path: `transitions[${index}].action`, label: `Editar ação ${index + 1}` }] : [])])];
  },
