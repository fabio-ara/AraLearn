...
export const stateTransitionTablePackage = Object.freeze({
  ...
  authoringContract: Object.freeze({
    ...
    example: Object.freeze({
      ...
      legend: "→ inicial · ◎ final ou aceitação",
  ...
  schema: Object.freeze({ type: "object", additionalProperties: false, required: ["states", "events", "transitions"], properties: {
    prompt: { type: "string" },
    stateColumnLabel: { type: "string", minLength: 1, maxLength: 120 },
    legend: { type: "string", minLength: 1, maxLength: 240 },
  ...
  normalize(data) {
    return { ...(data?.prompt ? { prompt: text(data.prompt) } : {}), stateColumnLabel: text(data?.stateColumnLabel) || "Estado", legend: text(data?.legend) || "→ inicial · ◎ final ou aceitação", states: (data?.states || []).map((state) => ({ id: text(state?.id), label: text(state?.label), initial: state?.initial === true, accepting: state?.accepting === true })), events: (data?.events || []).map((event) => ({ id: text(event?.id), label: text(event?.label) })), transitions: (data?.transitions || []).map((transition) => ({ id: text(transition?.id), from: text(transition?.from), event: text(transition?.event), to: text(transition?.to), ...(transition?.action ? { action: text(transition.action) } : {}) })) };
  },
  validate(data) {
  ...
  editableTargets(data) {
    return [...(data.prompt ? [{ path: "prompt", label: "Editar orientação" }] : []), { path: "stateColumnLabel", label: "Editar cabeçalho dos estados" }, ...data.states.map((_, index) => ({ path: `states[${index}].label`, label: `Editar estado ${index + 1}` })), ...data.events.map((_, index) => ({ path: `events[${index}].label`, label: `Editar evento ${index + 1}` })), ...data.transitions.flatMap((transition, index) => transition.action ? [{ path: `transitions[${index}].action`, label: `Editar ação ${index + 1}` }] : []), { path: "legend", label: "Editar legenda" }];
  },
