...
export function listSources(input = null) {
  ...
  if (input && typeof input === "object" && Array.isArray(input.sources)) {
    return listSources(input.sources);
  }
  return [];
}

export function listSourceSpans(input = null) {
  return listSources(input).flatMap((source, sourceIndex) =>
...
export function listSourceClaims(input = null) {
  return listSources(input).flatMap((source, sourceIndex) =>
...
export function buildSourceSpanMap(input = null) {
  const map = new Map();
  listSourceSpans(input).forEach((span) => {
    map.set(text(span?.spanId), span);
  });
  return map;
}

export function buildSourceClaimMap(input = null) {
  const map = new Map();
  listSourceClaims(input).forEach((claim) => {
    map.set(text(claim?.claimId), claim);
  });
  return map;
}

export function validateSourceLedger(input = []) {
  const errors = [];
  const seen = new Set();
  const seenSpanIds = new Set();
  const seenClaimIds = new Set();
  const sources = listSources(input).map((item, index) => {
  ...
  return {
    ...
    errors,
    sourceLedger: {
      ledgerId: text(input?.ledgerId) || "source-ledger",
      sources,
      summary: {
        sourceCount: sources.length,
        spanCount: listSourceSpans({ sources }).length,
        claimCount: listSourceClaims({ sources }).length
      }
    }
  };
}
