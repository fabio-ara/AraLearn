...
async function unresolvedOrphanMutationIds(transaction, courseId, normalizedRows) {
  const validLessonIds = new Set(normalizedRows.lessons.map((row) => String(row.id)));
  const validCardIds = new Set(normalizedRows.cards.map((row) => String(row.id)));
  const targetByEntity = new Map();
  for (const [storeName, targetField] of [
    ...
    const rows = await transaction.getAllByIndex(storeName, "byCourseId", courseId);
    for (const row of rows) {
      targetByEntity.set(`${storeName}:${row.id}`, row?.[targetField] ?? null);
    }
  }
  const outboxRows = await transaction.getAllByIndex("outbox", "byCourseId", courseId);
  return outboxRows.filter((row) => {
    if (!["pending", "inflight", "rejected"].includes(String(row.status || ""))) return false;
    const targetField = row.entityType === "lessonProgress"
      ? "lessonId"
      : ["cardProgress", "comments"].includes(row.entityType)
        ? "cardId"
        : "";
    if (!targetField) return false;
    const targetId = row.payload?.[targetField] ?? row.previousRow?.[targetField] ??
      targetByEntity.get(`${row.entityType}:${row.entityId}`);
    const validIds = targetField === "lessonId" ? validLessonIds : validCardIds;
    return !validIds.has(String(targetId || ""));
...
export class IndexedDbRelationalStore {
  ...
  async replaceOfficialCourseReplica(courseId, graph, {
    ...
    await this.transaction(stores, "readwrite", async (transaction) => {
      const blockingMutationIds = await unresolvedOrphanMutationIds(
        transaction,
        normalizedCourseId,
        normalizedRows
      );
    ...
    await this.transaction([
      ...
      for (const courseId of staleIds) {
        const unresolved = (await transaction.getAllByIndex("outbox", "byCourseId", courseId))
          .filter((row) => ["pending", "inflight", "rejected"].includes(String(row.status || "")));
    ...
    return this.transaction(
      ...
      async (transaction) => {
        const unresolved = (await transaction.getAll("outbox"))
          .filter((row) => ["pending", "inflight", "rejected"].includes(row.status));
        ...
        if (change.operation === "delete") {
          ...
          if (change.storeName === "courseSelections" && changeCourseId) {
            const unresolved = outboxRows.filter((row) =>
              ...
              ["pending", "inflight", "rejected"].includes(String(row.status || ""))
            );
            if (unresolved.length) {
              await transaction.put("syncState", {
