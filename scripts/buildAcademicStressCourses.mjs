...
function studyUnit({ id, position, title, content, response = null, feedbackItems = [], topics, sources }) {
...
const algorithms = moduleValue({
  ...
  microsequences: [
    ...
    microsequence({ id: "alg-representation", title: "Do procedimento ao estado", goal: "Ler código e rastreamento como representações complementares.", role: "explain", dependsOn: ["alg-foundation"], covers: ["alg-state", "alg-binary"], studyUnits: [
      studyUnit({ id: "alg-4", position: 1, title: "O procedimento em Python", content: [algorithmCode], topics: ["alg-binary"], sources: [SOURCES.ifspPpc] }),
      studyUnit({ id: "alg-5", position: 2, title: "O mesmo procedimento em execução", content: [algorithmTraceContent], topics: ["alg-state", "alg-binary"], sources: [SOURCES.ifspPpc] })
    ] }),
    microsequence({ id: "alg-practice", title: "Reconstruir e transferir", goal: "Praticar estado, ordem e invariantes em situações já fundamentadas.", role: "practice", dependsOn: ["alg-representation"], covers: ["alg-state", "alg-binary"], studyUnits: [
      studyUnit({ id: "alg-6", position: 1, title: "Complete o estado", content: [algorithmGapContent], response: gap("alg-6-response", algorithmGapContent.id, "rows[1][3]", "4", "text"), feedbackItems: [feedback("alg-6-feedback", "O índice central é 4 depois que o início passa a 3 e o fim permanece 5.")], topics: ["alg-state"], sources: [SOURCES.ifspPpc] }),
      studyUnit({ id: "alg-7", position: 2, title: "Ordene os gestos de uma iteração", content: algorithmOrderingContent, response: ordering("alg-7-response", algorithmOrderingContent), feedbackItems: [feedback("alg-7-feedback", "Primeiro se escolhe o centro; a comparação fornece a informação que permite reduzir o intervalo.")], topics: ["alg-binary"], sources: [SOURCES.ifspPpc] }),
...
const databases = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "db-foundation", title: "Três níveis que não são a mesma coisa", goal: "Situar domínio, esquema, transformação e instância antes das notações.", role: "explain", covers: ["db-conceptual", "db-relational", "db-transform"], studyUnits: [
      ...
      studyUnit({ id: "db-2", position: 2, title: "O desenho conceitual", content: [erDiagram], topics: ["db-conceptual", "db-transform"], sources: [SOURCES.ifspPpc] }),
      ...
      studyUnit({ id: "db-4", position: 4, title: "O esquema relacional", content: [relationalSchema], topics: ["db-relational", "db-transform"], sources: [SOURCES.ifspPpc] })
    ] }),
    microsequence({ id: "db-practice", title: "Transformar sem decorar", goal: "Praticar a função de entidade associativa, PK e FK.", role: "practice", dependsOn: ["db-foundation"], covers: ["db-conceptual", "db-relational", "db-transform"], studyUnits: [
      ...
      studyUnit({ id: "db-7", position: 3, title: "Preserve o fato histórico", content: [], response: choice("db-7-response", "Por que ITEM_PEDIDO conserva preco_venda se PRODUTO já possui preco_atual?", [{ id: "history", text: "Para preservar o preço efetivamente praticado quando o pedido foi feito." }, { id: "duplicate", text: "Porque toda tabela deve repetir todos os atributos relacionados." }, { id: "foreign", text: "Porque preço é sempre uma chave estrangeira." }, { id: "null", text: "Para permitir que o produto não tenha identificador." }], ["history"]), feedbackItems: [feedback("db-7-feedback", "Preço atual pode mudar; o item registra um fato histórico do pedido.")], topics: ["db-transform"], sources: [SOURCES.ifspPpc] })
    ] })
  ]
});
...
const networking = moduleValue({
  ...
  microsequences: [
    ...
    microsequence({ id: "tcp-representations", title: "Três perguntas, três representações", goal: "Usar layout de pacote, máquina de estados e topologia somente para os objetos que cada qual representa.", role: "explain", dependsOn: ["tcp-foundation"], covers: ["tcp-header", "tcp-state", "tcp-path"], studyUnits: [
      studyUnit({ id: "tcp-4", position: 1, title: "Onde a informação está no segmento", content: [tcpPacket], topics: ["tcp-header"], sources: [SOURCES.dataprev, SOURCES.tcp] }),
      studyUnit({ id: "tcp-5", position: 2, title: "Como eventos mudam a conexão", content: [tcpState], topics: ["tcp-state"], sources: [SOURCES.dataprev, SOURCES.tcp] }),
      studyUnit({ id: "tcp-6", position: 3, title: "Por onde a comunicação passa", content: [networkTopology], topics: ["tcp-path"], sources: [SOURCES.dataprev] })
    ] }),
...
const businessProcess = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "bpmn-foundation", title: "Antes dos símbolos", goal: "Situar processo, participante, responsabilidade e decisão.", role: "explain", covers: ["bpmn-responsibility", "bpmn-flow", "bpmn-gateway"], studyUnits: [
      ...
      studyUnit({ id: "bpmn-4", position: 4, title: "O processo completo", content: [bpmn], topics: ["bpmn-responsibility", "bpmn-flow", "bpmn-gateway"], sources: [SOURCES.dataprev, SOURCES.bpmn] })
    ] }),
