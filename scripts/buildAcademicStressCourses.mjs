...
function studyUnit({ id, position, title, content, response = null, feedbackItems = [], topics }) {
  return { id, position, title, role: response ? "practice" : "theory", content, response, feedback: feedbackItems, topics };
}
...
const algorithmTraceContent = algorithmTrace("ifsp-alg-trace");
...
const algorithms = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "alg-foundation", title: "Por que um algoritmo existe", goal: "Situar algoritmo como procedimento finito que transforma entrada em saída.", role: "explain", covers: ["alg-problem"], studyUnits: [
      studyUnit({ id: "alg-1", position: 1, title: "Antes do código", content: [paragraph("alg-1-text", "Imagine uma lista de nomes e a tarefa de descobrir se “Lia” aparece nela. O problema declara o que se deseja obter; um algoritmo descreve uma sequência finita e não ambígua de passos para produzir essa resposta a partir da lista recebida.")], topics: ["alg-problem"] }),
      studyUnit({ id: "alg-2", position: 2, title: "Entrada, estado e saída", content: [paragraph("alg-2-text", "A lista e o nome procurado são entradas. Durante a execução, posições e valores mudam: esse conjunto momentâneo é o estado. A posição encontrada, ou a indicação de ausência, é a saída. Separar essas três ideias permite compreender uma execução antes de dominar a sintaxe de uma linguagem.")], topics: ["alg-problem", "alg-state"] }),
      studyUnit({ id: "alg-3", position: 3, title: "Busca em lista ordenada", content: [paragraph("alg-3-text", "Se os valores estão ordenados, comparar o alvo com o elemento central informa em qual metade ele ainda pode estar. A busca binária conserva um intervalo possível e descarta apenas a metade que não pode conter o alvo.")], topics: ["alg-binary"] })
    ] }),
    microsequence({ id: "alg-representation", title: "Do procedimento ao estado", goal: "Ler código e rastreamento como representações complementares.", role: "explain", dependsOn: ["alg-foundation"], covers: ["alg-state", "alg-binary"], studyUnits: [
      studyUnit({ id: "alg-4", position: 1, title: "O procedimento em Python", content: [algorithmCode], topics: ["alg-binary"] }),
      studyUnit({ id: "alg-5", position: 2, title: "O mesmo procedimento em execução", content: [algorithmTraceContent], topics: ["alg-state", "alg-binary"] })
    ] }),
    microsequence({ id: "alg-practice", title: "Reconstruir e transferir", goal: "Praticar estado, ordem e invariantes em situações já fundamentadas.", role: "practice", dependsOn: ["alg-representation"], covers: ["alg-state", "alg-binary"], studyUnits: [
      studyUnit({ id: "alg-6", position: 1, title: "Complete o estado", content: [algorithmGapContent], response: gap("alg-6-response", algorithmGapContent.id, "rows[1][3]", "4", "text"), feedbackItems: [feedback("alg-6-feedback", "O índice central é 4 depois que o início passa a 3 e o fim permanece 5.")], topics: ["alg-state"] }),
      studyUnit({ id: "alg-7", position: 2, title: "Ordene os gestos de uma iteração", content: algorithmOrderingContent, response: ordering("alg-7-response", algorithmOrderingContent), feedbackItems: [feedback("alg-7-feedback", "Primeiro se escolhe o centro; a comparação fornece a informação que permite reduzir o intervalo.")], topics: ["alg-binary"] }),
      studyUnit({ id: "alg-8", position: 3, title: "Condição de uso", content: [], response: choice("alg-8-response", "Em qual situação a busca binária pode descartar metade dos candidatos com segurança?", [{ id: "ordered", text: "Os valores estão ordenados segundo o mesmo critério da comparação." }, { id: "distinct", text: "Todos os valores são necessariamente diferentes." }, { id: "short", text: "A lista contém menos de dez valores." }, { id: "memory", text: "A lista ocupa posições contíguas de memória." }], ["ordered"]), feedbackItems: [feedback("alg-8-feedback", "A ordenação é o fundamento lógico do descarte; tamanho, unicidade e posição física não bastam.")], topics: ["alg-binary"] })
    ] })
  ]
});
...
const databases = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "db-foundation", title: "Três níveis que não são a mesma coisa", goal: "Situar domínio, esquema, transformação e instância antes das notações.", role: "explain", covers: ["db-conceptual", "db-relational", "db-transform"], studyUnits: [
      studyUnit({ id: "db-1", position: 1, title: "Comece pelo domínio", content: [paragraph("db-1-text", "Uma escola precisa registrar estudantes, turmas e matrículas. Antes de pensar em tabelas, o modelo conceitual descreve quais objetos do domínio existem, quais propriedades importam e como eles se relacionam.")], topics: ["db-conceptual"] }),
      studyUnit({ id: "db-2", position: 2, title: "O desenho conceitual", content: [erDiagram], topics: ["db-conceptual", "db-transform"] }),
      studyUnit({ id: "db-3", position: 3, title: "Depois vem o modelo lógico", content: [paragraph("db-3-text", "No modelo relacional, cada relação possui atributos e uma chave que identifica tuplas. Uma chave estrangeira preserva uma referência entre relações. Ela não é uma seta genérica: seus valores devem corresponder à chave referenciada, salvo quando a nulabilidade permite ausência.")], topics: ["db-relational"] }),
      studyUnit({ id: "db-4", position: 4, title: "O esquema relacional", content: [relationalSchema], topics: ["db-relational", "db-transform"] })
    ] }),
    microsequence({ id: "db-practice", title: "Transformar sem decorar", goal: "Praticar a função de entidade associativa, PK e FK.", role: "practice", dependsOn: ["db-foundation"], covers: ["db-conceptual", "db-relational", "db-transform"], studyUnits: [
      studyUnit({ id: "db-5", position: 1, title: "Complete o relacionamento", content: [erGapContent], response: gap("db-5-response", erGapContent.id, "relationships[0].label", "realiza", "choice", ["contém", "substitui"]), feedbackItems: [feedback("db-5-feedback", "O rótulo deve formar uma proposição compreensível no domínio: estudante realiza matrícula.")], topics: ["db-conceptual"] }),
      studyUnit({ id: "db-6", position: 2, title: "Complete a função de cada símbolo", content: [databaseCorrespondenceContent], response: correspondenceGaps("db-6-response", databaseCorrespondenceContent), feedbackItems: [feedback("db-6-feedback", "Os três sinais descrevem propriedades distintas: identidade, referência e admissibilidade de ausência.")], topics: ["db-relational"] }),
      studyUnit({ id: "db-7", position: 3, title: "Preserve o fato histórico", content: [], response: choice("db-7-response", "Por que ITEM_PEDIDO conserva preco_venda se PRODUTO já possui preco_atual?", [{ id: "history", text: "Para preservar o preço efetivamente praticado quando o pedido foi feito." }, { id: "duplicate", text: "Porque toda tabela deve repetir todos os atributos relacionados." }, { id: "foreign", text: "Porque preço é sempre uma chave estrangeira." }, { id: "null", text: "Para permitir que o produto não tenha identificador." }], ["history"]), feedbackItems: [feedback("db-7-feedback", "Preço atual pode mudar; o item registra um fato histórico do pedido.")], topics: ["db-transform"] })
    ] })
  ]
});
...
const networking = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "tcp-foundation", title: "O problema resolvido pelo transporte", goal: "Situar aplicação, transporte e fluxo de bytes antes de mostrar números e siglas.", role: "explain", covers: ["tcp-service"], studyUnits: [
      studyUnit({ id: "tcp-1", position: 1, title: "Duas aplicações distantes", content: [paragraph("tcp-1-text", "Um processo envia bytes em um computador e outro processo precisa recebê-los em outro. IP encaminha pacotes entre hosts, mas não oferece sozinho à aplicação a conversa confiável, ordenada e identificada por portas que este exemplo exige.")], topics: ["tcp-service"] }),
      studyUnit({ id: "tcp-2", position: 2, title: "O que TCP oferece", content: [paragraph("tcp-2-text", "TCP apresenta às aplicações um fluxo confiável de bytes. Para sustentar essa abstração, numera bytes, confirma o que chegou, retransmite perdas e controla quanto pode permanecer em trânsito. O fluxo não preserva fronteiras das chamadas de envio da aplicação.")], topics: ["tcp-service"] }),
      studyUnit({ id: "tcp-3", position: 3, title: "Conexão não é um fio", content: [paragraph("tcp-3-text", "Uma conexão TCP é estado mantido nas duas extremidades: endereços e portas identificam os pontos, enquanto números de sequência, confirmações e janelas evoluem com os eventos. Pacotes intermediários podem seguir caminhos diferentes; a conexão é uma relação lógica entre os processos.")], topics: ["tcp-service", "tcp-state"] })
    ] }),
    microsequence({ id: "tcp-representations", title: "Três perguntas, três representações", goal: "Usar layout de pacote, máquina de estados e topologia somente para os objetos que cada qual representa.", role: "explain", dependsOn: ["tcp-foundation"], covers: ["tcp-header", "tcp-state", "tcp-path"], studyUnits: [
      studyUnit({ id: "tcp-4", position: 1, title: "Onde a informação está no segmento", content: [tcpPacket], topics: ["tcp-header"] }),
      studyUnit({ id: "tcp-5", position: 2, title: "Como eventos mudam a conexão", content: [tcpState], topics: ["tcp-state"] }),
      studyUnit({ id: "tcp-6", position: 3, title: "Por onde a comunicação passa", content: [networkTopology], topics: ["tcp-path"] })
    ] }),
    microsequence({ id: "tcp-practice", title: "Recuperar, distinguir e transferir", goal: "Praticar campos, estados e papéis sem exigir informação não ensinada.", role: "practice", dependsOn: ["tcp-representations"], covers: ["tcp-header", "tcp-state", "tcp-path"], studyUnits: [
      studyUnit({ id: "tcp-7", position: 1, title: "Localize a confirmação", content: [packetGapContent], response: gap("tcp-7-response", packetGapContent.id, "fields[3].label", "Número de confirmação", "choice", ["Porta de origem", "Ponteiro urgente"]), feedbackItems: [feedback("tcp-7-feedback", "O número de confirmação indica o próximo octeto esperado pelo receptor.")], topics: ["tcp-header"] }),
      studyUnit({ id: "tcp-8", position: 2, title: "Complete a representação de cada objeto", content: [networkCorrespondenceContent], response: correspondenceGaps("tcp-8-response", networkCorrespondenceContent), feedbackItems: [feedback("tcp-8-feedback", "Cada representação responde a uma pergunta diferente: estrutura binária, comportamento por estado ou conectividade física/lógica.")], topics: ["tcp-header", "tcp-state", "tcp-path"] }),
      studyUnit({ id: "tcp-9", position: 3, title: "Transfira para um caso novo", content: [], response: choice("tcp-9-response", "Um receptor recebeu continuamente até o byte 5000 e anuncia que espera o próximo. Qual campo expressa diretamente essa informação?", [{ id: "ack", text: "Número de confirmação" }, { id: "source", text: "Porta de origem" }, { id: "checksum", text: "Checksum" }, { id: "offset", text: "Data Offset" }], ["ack"]), feedbackItems: [feedback("tcp-9-feedback", "A confirmação é cumulativa e informa o próximo octeto esperado.")], topics: ["tcp-header"] })
    ] })
  ]
});
...
const businessProcess = moduleValue({
  ...
  microsequences: [
    microsequence({ id: "bpmn-foundation", title: "Antes dos símbolos", goal: "Situar processo, participante, responsabilidade e decisão.", role: "explain", covers: ["bpmn-responsibility", "bpmn-flow", "bpmn-gateway"], studyUnits: [
      studyUnit({ id: "bpmn-1", position: 1, title: "Um processo atravessa responsabilidades", content: [paragraph("bpmn-1-text", "Em um atendimento, cliente e organização participam do mesmo resultado, mas não executam as mesmas atividades. BPMN separa participantes em pools e pode subdividir a responsabilidade interna em raias. Pool e raia respondem a “quem?”, não a “em que ordem?”.")], topics: ["bpmn-responsibility"] }),
      studyUnit({ id: "bpmn-2", position: 2, title: "Duas espécies de fluxo", content: [paragraph("bpmn-2-text", "O fluxo de sequência liga elementos dentro de um participante e informa a ordem do processo. Entre participantes, o fluxo de mensagem representa comunicação. Misturar os dois apagaria a fronteira de responsabilidade que a notação foi criada para preservar.")], topics: ["bpmn-flow"] }),
      studyUnit({ id: "bpmn-3", position: 3, title: "A decisão exclusiva", content: [paragraph("bpmn-3-text", "Um gateway exclusivo escolhe um entre caminhos alternativos conforme condições. Ele não executa trabalho: apenas controla a divergência ou a convergência do fluxo. As condições pertencem às saídas, como “Sim” e “Não”.")], topics: ["bpmn-gateway"] }),
      studyUnit({ id: "bpmn-4", position: 4, title: "O processo completo", content: [bpmn], topics: ["bpmn-responsibility", "bpmn-flow", "bpmn-gateway"] })
    ] }),
    microsequence({ id: "bpmn-practice", title: "Ler sem decorar figuras", goal: "Praticar responsabilidade, decisão e tipo de fluxo no próprio diagrama.", role: "practice", dependsOn: ["bpmn-foundation"], covers: ["bpmn-responsibility", "bpmn-flow", "bpmn-gateway"], studyUnits: [
      studyUnit({ id: "bpmn-5", position: 1, title: "Complete a condição", content: [bpmnGapContent], response: gap("bpmn-5-response", bpmnGapContent.id, "flows[5].label", "Sim", "choice", ["Não", "mensagem"]), feedbackItems: [feedback("bpmn-5-feedback", "A condição “Sim” conduz da verificação de completude à emissão da decisão.")], topics: ["bpmn-gateway"] }),
      studyUnit({ id: "bpmn-6", position: 2, title: "Escolha o fluxo correto", content: [], response: choice("bpmn-6-response", "Qual fluxo deve representar a solicitação enviada do pool Cliente ao pool Organização?", [{ id: "message", text: "Fluxo de mensagem, porque cruza participantes." }, { id: "sequence", text: "Fluxo de sequência, porque toda seta indica sequência." }, { id: "association", text: "Uma associação sem direção, porque não há comunicação." }, { id: "gateway", text: "Um gateway paralelo, porque há dois pools." }], ["message"]), feedbackItems: [feedback("bpmn-6-feedback", "Fluxo de sequência permanece dentro do participante; a comunicação entre pools usa fluxo de mensagem.")], topics: ["bpmn-flow"] }),
      studyUnit({ id: "bpmn-7", position: 3, title: "Ordene o caminho quando os dados estão completos", content: bpmnOrderingContent, response: ordering("bpmn-7-response", bpmnOrderingContent), feedbackItems: [feedback("bpmn-7-feedback", "A ordenação reproduz apenas o caminho “Sim”; o retorno para complemento pertence ao outro ramo.")], topics: ["bpmn-gateway", "bpmn-flow"] })
    ] })
  ]
});

const project = {
  contract: "aralearn.course.v1", scope: "course", courses: [
...
const studyUnitCount = validation.value.courses.flatMap(({ modules }) => modules).flatMap(({ lessons }) => lessons).flatMap(({ microsequences }) => microsequences).flatMap(({ studyUnits }) => studyUnits).length;
console.log(`Cursos acadêmicos gerados em ${outputPath}: ${validation.value.courses.length} cursos, ${studyUnitCount} Unidades de estudo.`);
