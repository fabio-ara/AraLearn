...
switch ($Profile) {
  'GitHubPagesManagedSupabase' {
    ...
    Add-Step 'integrate-main' 'Integrar e validar a revisão' manual 'Conclua candidate:ready e o check protegido Testar e validar, depois integre a candidata por merge normal. Preserve o run certificado e sua tentativa exata; o publicador confere vínculo com o PR integrado, árvore, configuração e aplicabilidade. Pages é publicado somente quando o certificado o exigir e pelas fases explícitas de pages.yml.'
    Add-Step 'prepare-product' 'Preparar os artefatos da promoção' manual 'Com site e backend anteriores ainda correspondentes, execute preparar no SHA integrado. Aguarde assinatura histórica, conferência dos artefatos e prova nativa de instalação e upgrade. Registre o run e a tentativa desta preparação; ela não publica o produto nem comprova o backend novo.' `
      'gh workflow run pages.yml --ref main -f phase=preparar -f candidate_run_id=<run-certificado> -f candidate_run_attempt=<tentativa-certificada>'
