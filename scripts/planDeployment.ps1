...
switch ($Profile) {
  ...
  'StaticHostManagedSupabase' {
    ...
    Add-Step 'database-apply' 'Aplicar as migrations aprovadas' automatic 'Executa somente migrations versionadas, sem reset nem seed.' `
      "pwsh -NoProfile -File .\scripts\deploySupabase.ps1 -ProjectUrl $projectArgument -Mode Apply"
    Add-Step 'auth-urls' 'Cadastrar os endereços do aplicativo' manual "Use $applicationArgument/ como Site URL e permita somente os redirecionamentos realmente usados."
    Add-Step 'build' 'Gerar os arquivos estáticos' automatic 'Gera .pages com a configuração pública presente no processo.' 'npm.cmd ci; npm.cmd run pages:build'
  ...
  'LocalDevelopment' {
    ...
    Add-Step 'tests' 'Executar banco e aplicação' automatic 'Valida SQL, runtime e código sem usar credenciais hospedadas. Os smokes completos acrescentam uma fixture e a Edge Function conforme docs/implantacao.md.' `
      'npx.cmd --yes supabase@2.109.1 test db; npm.cmd test; npm.cmd run lint'
...
if ($IncludeAndroid) {
  ...
  Add-Step 'android-build' 'Gerar e analisar o APK' automatic 'Compila o APK e executa o lint Android. A release requer assinatura mantida fora do repositório.' `
    'npm.cmd run android:debug; .\android\gradlew.bat -p .\android :app:lintDebug --no-daemon'
