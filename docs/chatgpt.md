...
Desenvolva as explicações e fontes antes das unidades. Consulte os contratos dos componentes escolhidos. Resolva os impedimentos apresentados por preparar_materializacao e só materialize quando o resultado for ready, usando a mesma referência de preparo. Toda prática nova deve oferecer avaliação e feedback offline; response.open permanece apenas no conteúdo legado.
