...
| confinar a credencial OAuth do MCP ao recurso | Sim | O access token usa escopo exato `offline_access`, aliases pareados e assinatura ES256/EC P-256 verificada por JWKS; uma RPC resolve sessão e consentimento vivos | Proprietário que autorizou o cliente MCP | Smokes locais e hospedados exercitam código, refresh, MCP e negativas em GoTrue, API de dados e Storage; a janela de expiração e a repetição final foram concluídas | O fluxo não emite `id_token`, revogou consentimentos e sessões OAuth anteriores no corte e exige nova autorização | Sim | Alto | `aralearn_session_id` mantém o UUID correlacionável da sessão de origem, portanto a credencial inteira não é anônima; URLs temporárias continuam válidas até a própria expiração |
