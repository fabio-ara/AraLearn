...
validada de `main`. A candidata 0.0.27, ainda não publicada, eleva o catálogo
público de seis para cinco ferramentas e mantém o recurso visual MCP 0.0.23.
A candidata concluiu os gates locais de código, banco, concorrência, navegador,
OAuth e artefatos; integração, promoção e verificação hospedada permanecem
pendentes. A página distingue implementação, ligação entre camadas, acesso, uso
e evidência.
...
passaram. A sequência avança para a candidata 0.0.27 de fronteiras
operacionais e privacidade, sem antecipar a investigação Git. A Autoria expôs carga
...
| controlar dados da conta no dispositivo | Candidata | A sessão persistida contém tokens, tipo, expiração e `user.id`; Curso, cache e rascunhos ficam no namespace da conta | A própria pessoa em **Conta e aparência** | Testes de sessão, store local e navegador da revisão candidata; publicação pendente | Logout comum preserva somente dados, filas e rascunhos já persistidos; uma alteração aberta apenas no formulário exige confirmação de perda; limpeza explícita remove somente a conta ativa, com ou sem encerrar a sessão | Sim | Alto | Tokens continuam sendo segredos; limpar o dispositivo não apaga dados do servidor, e logout comum não equivale a limpeza local |
...
| confinar a credencial OAuth do MCP ao recurso | Candidata | O access token usa escopo exato `offline_access`, aliases pareados e assinatura ES256/EC P-256 verificada por JWKS; uma RPC resolve sessão e consentimento vivos | Proprietário que autorizou o cliente MCP | Smokes locais exercitam código, refresh, MCP e negativas em GoTrue, API de dados e Storage; promoção pendente | O fluxo não emite `id_token`, revoga consentimentos e sessões OAuth anteriores no corte e exige nova autorização | Sim | Alto | `aralearn_session_id` mantém o UUID correlacionável da sessão de origem, portanto a credencial inteira não é anônima; depois da promoção é obrigatório aguardar o maior prazo entre `jwt_expiry` e duas horas das URLs v1 de upload, mais margem, e repetir as negativas e o inventário antes de fechar a fronteira |
...
| manter perfil e compartilhar para Estudo | Sim | Perfil, avatar privado e acesso direto usam a API da aplicação; na revisão candidata, Pessoas deixa o MCP público | Proprietário concede ou revoga; favorecido recebe somente Estudo | Testes locais; a promoção candidata ainda está pendente | A concessão candidata responde de forma genérica, admite dez tentativas por ator em dez minutos e registra contadores sem e-mail ou hash; uma relação efetiva continua visível ao proprietário numa leitura posterior | Sim | Alto | Não há grupos, organizações, convite pendente ou coautoria; controlador, base jurídica e política institucional continuam externos ao código |
| excluir a própria conta | Sim | O aplicativo remove objetos e repete a função relacional; na chamada final candidata, o banco revoga sessões imediatamente antes do usuário Auth | Somente a própria pessoa, após confirmação literal | Testes de API, controlador e banco da candidata; promoção ainda pendente | Uma recusa remota por objeto preserva a sessão necessária à limpeza; depois do sucesso remoto, a exclusão é terminal e eventual bloqueio do IndexedDB admite apenas repetir a limpeza local | Sim | Alto | Uma URL de download já emitida pode permanecer válida por até 60 segundos; inventário não autoriza exclusão automática nem decide retenção de backup |
| registrar Observações situadas | Sim | Estudo e aplicação usam o DTO protegido completo; o MCP candidato recebe projeção minimizada e só inclui `rawText` com opt-in explícito | Estudante vê somente as próprias; proprietário recebe a caixa de entrada | Testes de banco, repositório, MCP, reconexão, duas abas e navegador da candidata; promoção pendente | Texto original e resposta permanecem na aplicação autorizada; inbox MCP omite texto, referência/rótulo protegidos, caminhos, links, IDs internos e horários | Sim | Alto | Exportação v2 continua pessoal ou pseudonimizada; ausência, quantidade, categoria e tempo de tratamento não diagnosticam aprendizagem |
...
| anexar PDF a uma Fonte | Sim | Na candidata, intenção privada de dez minutos, POST autenticado, confirmação transacional e vínculo à revisão da Fonte formam o fluxo | Proprietário com sessão viva | Testes de banco, cliente e Storage da candidata; a jornada hospedada ainda pertence ao fluxo anterior | PDF de até 20 MiB, no máximo oito por revisão de Fonte e 64 MiB de conteúdo único por Curso; v2 prepara somente upload autenticado e v1 permanece somente no download legado do Android 0.0.26 | Sim | Alto | O upload antigo falha fechado; retirar v1 exige decisão explícita de encerrar suporte ao 0.0.26, nunca detecção por `User-Agent`; download assinado dura 60 segundos |
...
| executar retenção por classe | Candidata | Uma função privada diária limpa, com limite e contagem próprios, Anotações retiradas, três famílias de recibo, intenções de PDF e janelas de concessão | Operação administrativa; nenhuma interface de estudante | Testes PGlite e smoke local da revisão candidata; promoção e observação hospedada pendentes | 512 itens por classe às 03:17, repetição idempotente e inventário separado de órfãos sem remoção | Sim | Alto | Prazos técnicos não definem retenção institucional de logs, backups, autoria ou pesquisa; MFA e proteção de senha permanecem decisão de implantação |
