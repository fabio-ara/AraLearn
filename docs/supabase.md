...
`list_catalog_collections` expõe somente metadados pesquisáveis de coleções e cursos oficiais publicados. Coleções não concedem escrita a usuários comuns. Trilhas pessoais usam `apply_study_path_mutation`: a RPC exige dispositivo e sessão, deduplica por `mutationId`, compara `baseRevision` e força `owner_id = auth.uid()`. `study_paths` e `study_path_courses` participam do feed e do snapshot atômico de `bootstrap_replica`; elas não fazem parte do contrato JSON v3.
