...
| contexto aplicado é derivado pelo servidor, não declarado pelo cliente | `courseDesignParameters.js`; `start` de materialização; hash do `designContext`; `designApplication` | testes de contexto multi-alvo, hash, rollback e retry | demonstrado com limite: catálogos selam `{id,position,statement,version}`, cada alvo referencia seus IDs e a etapa é auditada somente contra esse subconjunto |
