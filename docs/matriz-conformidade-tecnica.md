...
| uma estrutura persistente pode surgir sem caso de uso revisado | `auditVerticalParity.mjs`; registro canônico de Anotações, Estudo, Autoria, Fontes, pessoas/acesso, componentes e transportes; inventário exato versionado de relações, funções, índices, restrições, triggers, policies, estado RLS e buckets | `vertical-parity-audit.test.js`; comparação do PostgreSQL após `db reset` na CI | não oferecido: todo objeto novo falha até regeneração e revisão explícitas; o inventário pós-`20260817200000` contém 2.096 objetos, liga 501 aos sete casos correntes (`course-anchored-annotations` 84, `course-authoring-experience` 272, `course-source-provenance` 84, `study-course-experience` 26, `person-profile-and-course-access` 31, `didactic-component-runtime` 1 e `course-shared-transports` 3) e mantém 1.595 em `pre-course-database-removal`; `private.valid_course_personal_state_v1` é físico isolado, não capacidade do runtime v2 |
...
| Observações é a sétima área com caixa de entrada e links profundos | `CourseAuthoringSurface.js`; `CourseObservationsPanel.js`; `CourseInspectionSequence.js` | testes do painel, superfície, rota e viewports | demonstrado tecnicamente: sínteses/filtros/detalhe e criação autoral; Unidade é anotada a partir da Inspeção; compreensão humana ainda requer aceite |
...
| Anotações ancoradas implementam achados, correção e verificação | fronteira funcional posterior; preflight fail-closed de `audit_finding` | inventário de RPC/UI, migration `2000` e documentação | não oferecido: Observações reúnem manifestações e triagem; achados, correção, revisão e verificação independente exigem contrato próprio |
