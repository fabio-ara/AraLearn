...
| Observação | Autor da entrada; uma intenção ligada a um ou mais alvos, versões e bases de comparação. | **Observações**; registro, leitura, edição e `decidir_observacao`. Corrigir e confirmar a persistência conservam a pendência até a decisão expressa sobre os alvos apresentados. |
| Inspeção por IA | Proprietário autoriza a inspeção; parecer ligado à base efetivamente examinada. | `preparar_revisao` e `registrar_inspecao`. Mudanças materiais desatualizam o parecer; registrá-lo não edita conteúdo nem declara revisão humana. |
