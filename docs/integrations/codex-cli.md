...
O serviço `codex-cli` é um serviço HTTP local de integração com Codex CLI para
a assistência bottom-up. Ele
...
O serviço local recebe uma operação estruturada de revisão do aplicativo,
...
Planejamento e autoria extensa de cursos não passam por este serviço local.
Esse trabalho pertence ao GPT personalizado com Action ou a um cliente
compatível pela integração MCP.
...
ser enviado pelo app e configurado no serviço local; pedido sem token ou com token
...
O serviço local usa um comando fechado `codex exec -`; argumentos arbitrários não são
...
Para chamadas `codex exec`, o serviço local desativa shell, apps, navegador, controle
...
Nos provedores por API, o AraLearn monta uma requisição HTTP para o serviço
escolhido. Com Codex CLI, o aplicativo fala com o serviço HTTP local de
integração, que chama o binário do Codex.

```text
AraLearn -> serviço HTTP local -> Codex CLI -> saída estruturada -> compilação e validação atômica
```
...
- O serviço local envia todas as solicitações ao Codex via `stdin`; não grava o prompt
  em arquivo.
- Os arquivos temporários de schema e resposta são removidos antes de concluir
  tanto sucesso quanto falha.
- O schema exato entra no prompt e em `--output-schema`; o serviço local e o
  provider validam novamente a resposta.
...
O Codex CLI é acessado por um serviço HTTP local de assistência, não por um
backend alternativo de autoria. O aplicativo monta
o contexto, pede a saída no formato esperado e valida o fragmento. A autoria
extensa e a publicação remota usam o GPT personalizado com Action ou um cliente
compatível pela integração MCP; no
