...
| P2 | Quando condições, exigências e dificuldades previstas são explicitadas, respostas locais podem tornar a progressão mais coerente com o público e o objetivo sem reduzir profundidade. | `src/authoring/pedagogicalBlueprint.js`; `instructionalDesignContracts.js`; `designParameterResolution.js`; `instructionalDesignBinding.js`; `authoring/core/quality.md` | validação dos vínculos condição–exigência–dificuldade–resposta, resolução por escopo, binding do blueprint e diff factual entre plano e materialização | julgamento de especialistas e tarefas de explicação e aplicação sob condições e conhecimentos prévios delimitados | perguntar somente diante de lacuna material; dividir, recompor, aprofundar ou trocar a resposta quando houver promessa ausente, salto, condensação, fragmentação, prática sem base, representação inadequada ou meio externo indisponível |
| P3 | Quando a tarefa depende de estrutura disciplinar, representação especializada adequada pode reduzir ambiguidade evitável. | `src/resources/kernel/packageRegistry.js`; `src/resources/catalog/resourceCatalog.js`; `src/authoring/resourceSetResolution.js` | contrato válido, conjunto disponível versionado, seleção autorizada, uso materializado, geometria e descrição acessível | comparar texto, recurso inadequado e recurso especializado em tarefa equivalente, com especialista do domínio | fundir, restringir ou retirar quando a distinção não for compreendida ou não justificar o custo |
