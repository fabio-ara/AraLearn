...
| Direção editorial preserva conteúdo | estilo, títulos e extensão ficam separados dos quatro parâmetros pedagógicos | configuração por curso/microssequência | comparação autoral com repertório constante | limite editorial elimina novidade ou prática em vez de reorganizar unidades |
| Configuração distingue contexto de condição | `default` permite calibração pelo GPT; valor fixado pelo pesquisador prevalece e fica auditável no uso efetivo | configuração aplicada selada nas unidades e exportação de Analytics | comparação entre cursos independentes sob protocolo declarado | preset é tratado como universal ou condição fixada é silenciosamente recalibrada |
