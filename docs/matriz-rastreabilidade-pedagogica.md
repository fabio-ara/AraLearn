...
| P8 | Dúvidas e possíveis erros podem perder sentido quando separados do alvo que os provocou. | Canal externo, registro comportamental inferido ou observação voluntária situada. | A manifestação vira uma Anotação ancorada protegida; várias podem coexistir no mesmo alvo, chegar à caixa de entrada e receber resposta ou estado sem fingir reparo. | Avaliação formativa e *feedback literacy* tratam feedback como processo de interpretação e ação ([Nicol e Macfarlane-Dick (2006)](referencias.md#ref-nicol2006formative); [Carless e Boud (2018)](referencias.md#ref-carless2018feedbackliteracy); [Nicol e Kushwah (2024)](referencias.md#ref-nicol2024feedbackagency)). | A observação pode ser reencontrada por caminho e link profundo sem copiar todo o Curso; estudante vê somente as próprias. | Categoria, quantidade, ausência, estado, resposta e resolução não diagnosticam domínio, dificuldade, atenção, qualidade ou eficácia. |
...
| P13 | Uma única superfície que mistura estudo, planejamento, edição e revisão aumenta a quantidade de decisões simultâneas. | Navegação por páginas independentes, dashboard total ou destinos progressivos com o mesmo contexto. | Autoria separa Planejamento, Parâmetros, Fontes, Estrutura, Inspeção, Observações, Variantes e Pessoas, mantendo somente a tarefa corrente em primeiro plano. | Interfaces móveis dependem de contexto, dispositivo e continuidade da tarefa ([Ahmad Faudzi et al. (2023)](referencias.md#ref-faudzi2023mobileui)); interrupções impõem custo de retomada ([Monk et al. (2008)](referencias.md#ref-monk2008resumption)). | Cada destino preserva contexto, retorno e disclosure compatíveis com sua tarefa. | Paridade automatizada e ausência de overflow não demonstram descoberta, compreensão ou usabilidade humana. |
| P14 | Comparar Cursos sem uma base comum ou diferenças declaradas permite confundir mudanças planejadas e acidentais. | Duplicar cursos livremente, gerar produto fatorial implícito ou criar Cursos independentes a partir de checkpoint, diferenças e desvios explícitos. | Variantes locais registram checkpoint do planejamento, parâmetros/política declarados, revisões e materialização de Cursos independentes; a comparação permanece descritiva. | Comparações interpretáveis exigem desenho, mensuração e análise compatíveis; reprodução técnica é necessária, mas não prova causalidade. | O planejamento comum e as diferenças declaradas podem ser confrontados sem apagar a identidade autônoma de cada Curso. | Não demonstra equivalência dos grupos, exposição, aderência, validade do outcome nem efeito educacional. |
...
| P8 | Ao surgir uma dúvida situada, Anotação ancorada e retorno no mesmo contexto podem apoiar decisão de revisão. | `courseAnchoredAnnotations.js`; folha no Estudo; caixa de entrada na Autoria | N por alvo, identidade/caminho estáveis, self-only, offline, resposta e estados sem vínculo fictício de reparo | registrar, reencontrar, interpretar o retorno e decidir ação; analisar casos negativos | revisar se o contexto se perder, a manifestação virar diagnóstico ou a responsabilidade ficar indefinida |
...
Variantes comparáveis locais registram somente a linhagem e diferenças de
Curso. Locks, atribuição e entrega privada não são capacidade corrente; mesmo
quando entrarem, reprodução técnica não demonstrará comparabilidade empírica,
validade do outcome, aderência ou relação causal.
