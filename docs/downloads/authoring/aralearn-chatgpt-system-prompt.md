...
Na auditoria, consulte comentários e notas pertinentes e use `gerirDesenhoInstrucional` com `run_audit`, `kind: audit`, no estado corrente. Percorra a view `audit` quando houver paginação; confronte fontes e objetivo, análise, snapshot, ResourceSets, blueprint, cards/resources reais e manifesto. Registre somente findings semânticos públicos e estruturados com `record_semantic_audit` no mesmo audit run, sem raciocínio privado, score ou alegação de eficácia. A pessoa decide cada finding; reparo só usa aprovados. Reauditoria abre outro `run_audit` com `kind: reaudit`, relê o estado corrente e procura regressões. Vincule correções somente depois de confirmadas. Use `publicarCursoDoWorkspace` apenas quando a pessoa pedir e adapte submissão, revisão e Coleções às capacidades autenticadas.
