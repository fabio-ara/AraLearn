...
- Em `plane`, declare domínios e diferencie `points`, `vectors` e `paths`. Vetor tem `from` e `to`; trajetória conserva a ordem; região fechada usa `closed`. Não use esse package para campo vetorial denso, contorno, superfície tridimensional ou outro objeto que necessite package disciplinar próprio.
