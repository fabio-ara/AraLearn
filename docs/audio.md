...
O áudio pode integrar uma unidade de estudo ou a [explicação compartilhada](explicacao-e-revisao-humana.md) de uma microssequência. Abra o ícone da ferramenta para escolher uma faixa, reproduzir, parar ou consultar a alternativa textual disponível. Cada ferramenta pode reunir faixas com nomes e idiomas diferentes. O conteúdo em que ela aparece deve explicar sua função: ouvir um exemplo, reconhecer um som ou acompanhar uma explicação, por exemplo.
...
Os formatos aceitos são WAV com PCM inteiro, que guarda amostras de som sem compressão, e MP3, que usa compressão. O limite por arquivo é 20 MiB; a cota de arquivos do curso reúne PDFs e áudios e totaliza 64 MiB de conteúdo único. O servidor confere os bytes e o acesso. O curso guarda tamanho, formato e um hash, resumo calculado para conferir a integridade do arquivo. O endereço interno do armazenamento não integra o conteúdo. Ao reproduzir, o aplicativo confere o arquivo recebido e cria um objeto em memória, chamado Blob; o endereço local desse objeto é liberado ao terminar seu uso. Um arquivo com estrutura aceita ainda pode falhar no decodificador de um navegador; nesse caso a ferramenta informa a falha e permite tentar novamente.

Em um curso privado, a pessoa proprietária consulta a biblioteca inteira; uma pessoa com acesso compartilhado obtém apenas arquivos associados à unidade ou explicação que abriu e cujo conteúdo pode acessar. Para visitantes de um curso público, o arquivo também precisa estar associado ao conteúdo aberto e o proprietário precisa ter confirmado a disponibilidade pública dos arquivos do curso. Tornar o curso público, sozinho, não libera os áudios. Um arquivo solto na biblioteca não vira um recurso público por existir no mesmo curso.
...
O aplicativo aceita até 16 mil caracteres por fala; o fornecedor também impõe seu limite de contexto. O modelo documenta 8.192 tokens de entrada e 16.384 de saída. Tokens são as unidades pelas quais o serviço processa texto ou áudio; não correspondem a uma quantidade fixa de caracteres. Por isso, um texto dentro do limite local ainda pode exceder a capacidade do serviço. Divida a gravação por partes que façam sentido na aprendizagem; o aplicativo não corta o texto automaticamente. [Limites do modelo](https://ai.google.dev/gemini-api/docs/models/gemini-2.5-flash-preview-tts).
