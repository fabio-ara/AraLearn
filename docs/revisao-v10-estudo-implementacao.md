...
- **Associação de arestas:** máquina de estados, BPMN e contêineres compartilham label + decorate. A polilinha do vínculo recebe o mesmo traço da aresta. O halo da máquina de estados foi compartilhado com textos de fluxos e objetos BPMN. O espaçamento BPMN foi ajustado após reproduzir a colisão real com gateway na CI; o custo de área está documentado abaixo. Não há coordenadas, curvas ou cores novas no contrato autoral.
...
| H009, O073 | BPMN sem colisões e vínculo inequívoco | Espaçamento, label + decorate e halo compartilhado | Sobreposição de f7 com gateway: 36,7% → zero; vínculo com própria spline; dois hosts; pan/zoom/tela inteira | Área do exemplo aumentou cerca de 26%; casos delimitados | Corrigido após falha na CI; testes focais verdes |
...
A/B sobre o motor embarcado: label + decorate teve zero colisões nos primeiros casos densos observados e um vínculo por aresta rotulada; xlabel + decorate colidiu com um participante no BPMN denso. No caso sintético reproduzível, o rótulo “confirmação registrada” cruzava “Atendimento ao público” com xlabel. A CI ampliada depois revelou outro caso: 36,7% da caixa de “dados recebidos” era coberta por um gateway. A primeira amostra não sustentava uma conclusão geral de ausência de colisões.

Decorate desenha a ligação geométrica até a spline; xlabel é posicionado depois do layout e não reserva o mesmo espaço. Cor não é o único mecanismo de associação. [Documentação Graphviz: decorate](https://graphviz.org/docs/attrs/decorate/), [xlabel](https://graphviz.org/docs/attrs/xlabel/).

No BPMN, `nodesep/ranksep` passaram de 0.5/0.7 para 1.2/1.3, a menor combinação bem-sucedida dentre as comparadas. O viewBox passou de 566 × 1094 para 578 × 1343, cerca de 26% a mais de área. A alternativa 1.5/1.6, com cerca de 47% a mais, foi descartada; 1.1/1.3 ainda colidia. Não foi feita comparação sistemática de diferentes limites de wrapping, nem se afirma ótimo global. O espaçamento da máquina de estados permaneceu igual. Halo protege texto quando traços passam por trás, inclusive nos eventos BPMN próximos das bordas de participantes/raias; não substitui a ausência de colisão com nós preenchidos. A sonda final mediu zero sobreposição de f7 em 390/1280. Self-loop, guarda/ação, rótulos longos e exploração permanecem cobertos pelos testes de diagramas.
...
Achados novos corrigidos: extremos descendentes dentro dos segmentos de memória; erro de unidade do tempo de voz nativa; corrida de pausa antes da resolução de play; cruzamento entre série e rótulo de referência no gráfico; colisão BPMN entre rótulo e gateway e traço de raia atravessando texto de evento. As provas finais de gráfico e BPMN substituem as capturas anteriores desses mecanismos. Asserções antigas sobre decoração de alternativas também foram atualizadas, preservando geometria, tipografia, área de toque e jornada de preenchimento/avanço.
...
| BPMN H009 + diagramas v10 após correção da CI | 17/17 | Quatro H009 e treze v10; headless local |
| Sondas finais BPMN | 1/1 catálogo nos dois hosts; 2/2 inspeção f7 em 390/1280 | Sondas de pixels, não persistência |
| Lacuna: clique mantido preserva geometria | 1/1 | Fixture local, pressionamento/preenchimento inspecionados |
| Contratos/Graphviz/tipografia após ajuste BPMN | 26/26 | Antes da extensão do mesmo halo aos textos dos nós |
| Catálogo 390 claro após ajuste geométrico BPMN | 1/1, 168 capturas, overflow zero, alvos ≥ 44 px | A captura BPMN foi renovada novamente após o halo final |
| ESLint focal e diff check | Passaram nas etapas anteriores | Gate final repetido após novos bytes |
...
No commit `fc9170c611b42d8816b4cb572283ac61edde636c`, a preparação canônica passou nos seis gates: preflight, lint, contratos, runtime (2.480 passaram, 15 ignorados), frontend focal (116 passaram) e Android. A [CI protegida 36263006642](https://github.com/fabio-ara/AraLearn/actions/runs/36263006642), tentativa 1, ampliou a suíte web para 554 casos: 539 passaram, dez foram ignorados e cinco falharam. Quatro pertenciam ao H009; um comparava a decoração antiga de lacunas. Preparação, Supabase local e Android passaram. A PR voltou a rascunho; a correção e as provas acima não reutilizam esse resultado como aprovação de CI. Os bytes finais precisam passar novamente pelo gate canônico e pela CI protegida.
...
Novo gate local/CI após a correção encontrada na primeira CI, publicação no ambiente autorizado e gravação do schema candidato na configuração funcional de Actions. A verificação somente de leitura do backend encontrou 236 migrações pareadas e nenhuma pendência/divergência; não aplicou migração nem publicou funções. Depois, autoria integral do curso novo pelo ChatGPT Chat, jornadas reais e distintas MCP/Actions, inspeção do destino e correção pelas ferramentas, auditoria das oito dimensões, cobertura do catálogo e substituição segura do legado.
