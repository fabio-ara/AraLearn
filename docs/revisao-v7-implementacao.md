...
| `20260924175938_revisao_v7_focal_audit_basis.sql` | Base focal da auditoria, invalidação por mudanças relevantes e dependências do foco produzido |
...
A [CI seguinte](https://github.com/fabio-ara/AraLearn/actions/runs/36090543680) aprovou web e Android, mas recusou a integração: o cenário de retenção ainda enviava o parecer anterior à v7, sem os cinco checks fundamentados. A fixture agora usa a base devolvida pelo servidor e declara que seu parecer é sintético; não representa uma avaliação pedagógica independente. A prova com PostgreSQL, Auth, Storage e Edge Functions locais reais conservou os arquivos entre bases compartilhadas, aprovação parcial e última aprovação, recuperou uma limpeza interrompida, impediu a ressurreição por pedido obsoleto e removeu o órfão pela Storage API.

A execução da jornada autoral seguinte encontrou outra fixture antiga, que enviava `parte` e duas microssequências na mesma chamada. O cenário passou a produzir cada microssequência separadamente e a concluir a parte depois de cobrir ambas. A substituição e a repetição continuam verificando a mesma revisão sem duplicação, as configurações explícitas do autor, as fontes, a exportação e as observações com múltiplos alvos. Essa jornada também passou no backend local real e removeu sua conta e seu curso sintéticos. Essas provas corrigem os consumidores dos contratos; não alteram nem relaxam as guardas do produto.

A prova seguinte, com partes de três microssequências encadeadas, revelou uma falha de produto: o SQL cobrava os pré-requisitos dos focos futuros da parte durante a primeira gravação parcial. Na cadeia A→B→C, exigir B para gravar A impedia o fluxo focal. O pacote v7 ainda não publicado passou a conferir as dependências dos alvos atuais na produção parcial e toda a parte na conclusão. A regra continua exigindo que o pré-requisito esteja materializado ou integre os alvos da gravação; não libera saltos na cadeia. Os dez [testes SQL de materialização incremental](../tests/runtime/incremental-materialization-pglite.test.js) passaram: a regressão cobre a sequência, a recusa de C sem B, a conclusão prematura e a preservação de A/B; o controle sem o bloco corretivo reproduz a falha original. Esse recorte PGlite usa stubs explícitos de acesso e composição; a prova seguinte usa o backend completo.
