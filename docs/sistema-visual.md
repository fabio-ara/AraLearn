...
Na moldura móvel, as ações globais à direita formam uma única linha vertical. **Conta e aparência** e o ingresso por convite terminam na mesma aresta, inclusive quando a tela reserva uma calha para a barra de rolagem. A troca de tela não pode deslocar o botão de conta nem horizontal nem verticalmente; todos os cabeçalhos reservam a mesma altura, mesmo quando um deles contém o seletor de modos. A ação de voltar conserva, de forma independente, a aresta esquerda dos cards. **Abrir curso** não pertence a esse eixo global: repete tamanho e recuos internos das ações dos demais cards de navegação.
